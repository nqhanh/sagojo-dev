...
x.css( {
    opacity: +a.opacity, cursor:a.overlayClose?"pointer":q;
}
).show();
    ...
    x.css( {
    width: n.width(), height:n.height(), top:n.scrollTop(), left:n.scrollLeft();
}
);
}).trigger("scroll."+P);
    w(fb, a.onOpen);
...
d.load(e);
    ...
    transition: "elastic", speed:300, width:c, initialWidth:"600", innerWidth:c, maxWidth:c, height:c, initialHeight:"450", innerHeight:c, maxHeight:c, scalePhotos:e, scrolling:e, inline:c, html:c, iframe:c, photo:c, href:c, title:c, rel:c, opacity:.9, preloading:e, current:"image  {
    current;
}
 of  {
    total;
}
", previous: "previous", next:"next", close:"close", open:c, returnFocus:e, loop:e, slideshow:c, slideshowAuto:e, slideshowSpeed:2500, slideshowStart:"start slideshow", slideshowStop:"stop slideshow", onOpen:c, onLoad:c, onComplete:c, onCleanup:c, onClosed:c, overlayClose:e, escKey:e, arrowKey:e;
...
;
    ...
    B=f("Wrapper");
    s=f("Content").append(k=f(M, "width: 0;
     height: 0;
     overflow: hidden"), K=f("LoadingOverlay").add(f("LoadingGraphic")), Z=f("Title"), Y=f("Current"), I=f("Next"), H=f("Previous"), F=f("Slideshow").bind(fb, jb), ab=f("Close"));
...
, function() {
    ...
    z=k.outerWidth(e);
    j.css( {
    "padding-bottom": C, "padding-right":D;
}
).hide();
    I.click(d.next);
    H.click(d.prev);
...
k.css( {
    ...
    Z.show().html(a.title);
    if(m>1) {
    typeof a.current==="string"&&Y.html(a.current.replace(/\ {
    current\;
}
/, g+1).replace(/\ {
    total\;
...
();
    ...
    J.hide();
    b("#"+i+"Photo").css( {
    cssFloat: t, marginLeft:q, marginRight:q;
}
);
...
n=a.href;
    ...
    j.one(hb, function() {
    var c=b("<iframe name='"+(new Date).getTime()+"' frameborder=0"+(a.scrolling?"": " scrolling='no'")+(A?" allowtransparency='true'":"")+" style='width:100%;
     height: 100%;
     border: 0;
     display: block;
    '/>");
    ...
    c.id=i+"Photo";
    b(c).css( {
    border: t, display:"block", cssFloat:"left"}
);
    if(a.scalePhotos) {
    s=function() {
    c.height-=c.height*e;
