...
function html_sitemap_shortcode_handler( $args, $content = null )
{
	...
	$args['echo'] = 0;
	$args['title_li'] = '';
	/*$args['exclude']  =  '17,19,25,22,273,284,275,2,24,23,148,604,636,667,690,751,754,780,979';*/
	/*$args['include']  = '242,21,258,245,440,20,683,6,171,100,1390';*/
	$args['include']  = '242,21,258,245,440,20,683,6,171,100,2741';
	...
	$sub = '';
	$child_list = get_categories('child_of=124&hide_empty=0');
	$sub .= '<li><a href="'.get_category_link(124).'">'.get_cat_name(124).'</a>';
	$sub .= '<ul class="children">';
	foreach($child_list as $child) {
		$sub .= '<li><a href="' . get_category_link( $child->term_id ) . '"' . '>' . __($child->name).'</a></li>';
	}
	$sub .= '</li></ul>';
	return '<ul>'. $html . $sub. '</ul>';
}
