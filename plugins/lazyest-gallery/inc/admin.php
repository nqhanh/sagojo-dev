...
class LazyestAdmin extends LazyestGallery {
  ...
  function update( $options ) {
      ...
      if ( isset( $options['gallery_folder'] ) ) {
		    ...
		    $options['gallery_folder'] =  rtrim( trim( str_replace( '\\', '/', $options['gallery_folder'] ) ), '/' ) . '/';
		  }
   ...
   * Lazyest Gallery roles should not be selectable in user screen.
   * 
   * @param array $all_roles
   * @since 1.1.9
   * @return array roles without lazyest gallery roles
   */
  function editable_roles( $all_roles ) { 
  	foreach( $all_roles as $key => $role ) {
  		if ( in_array( $key, array( 'lazyest_manager', 'lazyest_editor', 'lazyest_author' ) ) ) {
  			unset( $all_roles[$key] );
  		}
  	}
  	return $all_roles;
