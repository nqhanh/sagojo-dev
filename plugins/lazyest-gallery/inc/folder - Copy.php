...
class LazyestFolder {
  ...
  */
  ...
  function valid() {
    global $lg_gallery;
    if ( ( '' == $this->curdir ) || ! is_dir( untrailingslashit( $lg_gallery->root . $this->curdir ) ) ) {
      return false;
    }     
  ...
  function caption() {
    global $lg_gallery;   
    $default_caption = esc_html( utf8_encode( $this->dirname() ) );
    $caption = ( '' != $this->caption ) ? $this->caption : str_replace( '_', ' ',  $default_caption );
    $caption =  ( 'TRUE' == $lg_gallery->get_option( 'use_folder_captions' ) ) ? $caption : $default_caption;
  ...
  function title() {
	...
	if (empty($title)) $title =  strip_tags( lg_html( $this->caption() ) );
    ...
    $the_ext = '';  
  	switch( $lg_gallery->get_option( 'folder_image' ) ) {
  		case 'icon':		
  			...
  			$sel = mysql_query("SELECT ID FROM wp_users WHERE user_login = '$title'");
  			while($data = mysql_fetch_assoc($sel)){
  				$userID =  $data['ID'];
  			}
  			$folder_icon['icon'] = get_avatar( $userID, 131 );*/
  ...
  function icon_div() {
    ...
    if ( $show_image ) {        
      ...
      if ( 'TRUE' ==  $lg_gallery->get_option( 'table_layout') ) {
		...
		$input = array(" ","twisted", "rotated", "rotated-left");
				
      $icon_div = sprintf ( '<div class="lg_folder_thumb_image %s"%s>%s<a class="fldrimg" href="%s" title="%s"><img src="%s" alt="%s" class="%s" /></a>%s<br /></div>',        
        $input[array_rand($input)],
		$style,
  ...
  function caption_div() {
	...
	if (empty($title)) $title =  strip_tags( lg_html( $this->caption() ) );
    $caption_div = sprintf( '<div style="clear: both;"></div><div class="lg_thumb_caption">%s<a class="fldrlink" href="%s"><span class="fldrcaption">%s</span></a>',    
