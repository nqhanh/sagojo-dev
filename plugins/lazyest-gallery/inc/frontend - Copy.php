...
class LazyestFrontendFolder extends LazyestFolder {
  ...
  function thumb_image( $image ) {  	
    ...
    if ( 'TRUE' != $lg_gallery->get_option( 'enable_cache' )  || 
		...
		$moderator = "<a href = \"javascript:void(0)\" onclick = \"document.getElementById('".$image->name()."').style.display='block';document.getElementById('".$image->name()."').style.bottom ='27px';document.getElementById('fade".$image->name()."').style.display='block'\"\">Edit</a>
    	...
    	$delete = "<a href = \"javascript:void(0)\" onclick = \"document.getElementById('dlt".$image->name()."').style.display='block';document.getElementById('dlt".$image->name()."').style.bottom ='27px';document.getElementById('fadedlt".$image->name()."').style.display='block'\"\"> | Delete</a>
  ...
  function folder_header() {
    $title =  strip_tags( lg_html( $this->caption() ) );
	$sel = mysql_query("SELECT display_name FROM wp_users WHERE user_login = '$title'");
