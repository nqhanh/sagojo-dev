...
class LazyestFrontend extends LazyestGallery {
  ...
  function scripts() {   
    ...
    wp_enqueue_script( 'lg_gallery', $this->plugin_url . "/js/lazyest-gallery.$j", array( 'jquery' ), '1.1.20', true );
    
    wp_localize_script( 'lg_gallery', 'lazyest_ajax',    $this->localize_lazyest() );    
 		wp_localize_script( 'lg_gallery', 'lazyest_widgets', $this->localize_widgets() );
    wp_localize_script( 'lg_gallery', 'lazyestshow',     $this->localize_show() );
    
    if ( 'TRUE' == $this->get_option( 'async_cache' ) ) {
      wp_localize_script( 'lg_gallery', 'lazyestimg', $this->localize_loader() );
    }
  ...
  function localize_widgets() {
	return array( 
		'_nonce'             => wp_create_nonce( 'lazyest_widgets' ),
    'ajaxurl'            => admin_url( 'admin-ajax.php' ),
		'slideshow_duration' => apply_filters( 'lazyest_widgets_slideshow_duration', 1000 * intval( $this->get_option( 'slide_show_duration' ) ) ) );
	}
  ...
  function valid() { 
    ...
    $this->currentdir = ( is_dir( untrailingslashit($this->root . $this->file) ) ) ? ltrim( $this->file, '/' ) :  trailingslashit( ltrim( $path['dirname'], '/' ) );
    return true;
  }
   ...
   */
  function _is_dir() {
    return is_dir( untrailingslashit( $this->root . $this->file ) );
  }
  ...
  function show( $count = -1, $cols = -1 ) { // builds main gallery page
    ...
    if ( '' != $this->file ) {      
      ...
      if ( ! $folder->valid() ) {  
		...
		$is_slide =  $this->is_image( $this->file ); 
		$class = $is_slide ? 'slide' : '';
    echo "<div class='lg_gallery $class'>\n";
  ...
  function create_navigation() {
	...
	$user_login = wp_get_current_user()->ID;
	
	$myalbum = "<div class='myalbum'><a href=".site_url()."/kaisha-camera/?file=".$user_login.">&rarr;&nbsp;My album</a></div>";
	/*$title =  strip_tags( lg_html( $this->caption() ) );
		if ($title == $user_login) {
    ...
    $navigator = sprintf( '<div class="breadcrumbs">%s <a href="%s">%s</a> <span class="raquo">%s</span> <a href="%s">%s</a>%s',
      $now_viewing,
      home_url(), 
      /*get_bloginfo( 'name' ),*/
	  $home,
...
class LazyestFrontendFolder extends LazyestFolder {
  ...
  function thumb_image( $image ) {  	
    ...
    if ( 'TRUE' != $lg_gallery->get_option( 'enable_cache' )  || 
		...
		if ( 'nothing' != $lg_gallery->get_option( 'on_thumb_click' ) ) {
			$anchor_1 = sprintf('<a id="%s_%s" href="%s" class="%s" %s title="%s" >', 
				$onclick['id'],
	      $postid,
	      $onclick['href'],
	      $onclick['class'],
		...
		/*$moderator = "
   		...
   		j(document).ready(function(){
	   		j(\"#ajaxButton\").click(function(){
	 	  		 j.ajax({
	   			   var thought = document.getElementById('cmt".$image->name()."').val();
	 			   type: \"POST\",
	 			   url: \"".site_url()."/wp-content/plugins/lazyest-gallery/inc/update_comment.php\",
	 			   data: { 
	 					  	 'name': '".$image->name()."',
	 						 'str': thought		 
	 					  },
		...
		$moderator = "<a href = \"javascript:void(0)\" onclick = \"document.getElementById('".$image->name()."').style.display='block';document.getElementById('".$image->name()."').style.bottom ='26px';document.getElementById('fade".$image->name()."').style.display='block'\"\">Edit</a>

        <div id=\"".$image->name()."\" class=\"white_content\"><textarea rows=\"4\" id=\"cmt".$image->caption()."\" name=\"cmt".$image->caption()."\">".$image->comment(). "</textarea>
   		
  		<div style=\"text-align:center;padding-top:5px;\"><a class=\"gallery_button\" href = \"javascript:void(0)\" onClick=\"".$image->caption()."('".$image->name()."');document.getElementById('".$image->name()."').style.display='none';document.getElementById('fade".$image->name()."').style.display='none';\">Update</a>";
		
    	$moderator .= "&nbsp;&nbsp;<a class=\"gallery_button\" href = \"javascript:void(0)\" onclick = \"document.getElementById('".$image->name()."').style.display='none';document.getElementById('fade".$image->name()."').style.display='none';\">Close</a></div></div>
        <div id=\"fade".$image->name()."\" class=\"black_overlay\"></div>";
    	
    	$moderator .= "<script tye=\"text/javascrip\">
		    function ".$image->caption()."(name)
		    {
		    var xmlhttp;
		   	var str =  document.getElementById(\"cmt".$image->caption()."\").value;    		
		    if (window.XMLHttpRequest)
			  ...
			  xmlhttp=new ActiveXObject(\"Microsoft.XMLHTTP\");
				    ...
				    xmlhttp.open(\"GET\",\"".site_url()."/wp-content/plugins/lazyest-gallery/inc/update_comment.php?q=\"+name+\"&c=\"+str,true);
				    xmlhttp.send();
    				setTimeout(function(){
    	...
    	$delete = "<a href = \"javascript:void(0)\" onclick = \"document.getElementById('dlt".$image->name()."').style.display='block';document.getElementById('dlt".$image->name()."').style.bottom ='26px';document.getElementById('fadedlt".$image->name()."').style.display='block'\"\"> | Delete</a>
    		<div id=\"dlt".$image->name()."\" class=\"del_content\">&nbsp;&nbsp;<img src='".site_url()."/wp-content/plugins/lazyest-gallery/images/warning.png' width='24px'>&nbsp;&nbsp;Are you sure?
			<div style=\"text-align:center;padding-top:5px;\"><a class=\"gallery_button\" href = \"javascript:void(0)\" onClick=\"dlt".$image->caption()."('".$image->name()."');document.getElementById('dlt".$image->name()."').style.display='none';document.getElementById('fadedlt".$image->name()."').style.display='block';\">Yes</a>";
					
    	$delete .= "&nbsp;&nbsp;&nbsp;&nbsp;<a class=\"gallery_button\" href = \"javascript:void(0)\" onclick = \"document.getElementById('dlt".$image->name()."').style.display='none';document.getElementById('fadedlt".$image->name()."').style.display='none';\">No</a></div></div>
   		<div id=\"fadedlt".$image->name()."\" class=\"black_overlay\"></div>";
    	 
    	$delete .= "<script tye=\"text/javascrip\">
		    function dlt".$image->caption()."(name)
		    {
		    var xmlhttp;
      		var str =  '".$title."';
		    if (window.XMLHttpRequest)
			  ...
			  xmlhttp=new ActiveXObject(\"Microsoft.XMLHTTP\");
				    ...
				    xmlhttp.open(\"GET\",\"".site_url()."/wp-content/plugins/lazyest-gallery/inc/delete_img.php?q=\"+name+\"&c=\"+str,true);
				    xmlhttp.send();
		    }
	    	</script>";
    	}
    	
    return sprintf( '<a href="%s" rel="lightbox[roadtrip]" title="%s"><img class="%s" src="%s" alt="%s" /></a>%s<div>%s%s</div>',
	  $onclick['href'],
	  $image->comment(),                
      $class,
      $onclick['href'],
    ...
    return sprintf( '<a href="%s" rel="lightbox[roadtrip]" title="%s"><img class="%s" src="%s" alt="%s" /></a>%s',
    		/*$anchor_1,*/
	  $onclick['href'],
	  $image->comment(),                
      $class,
      $onclick['href'],/*$image->src(),*/
      $image->alt(),
	  $anchor_2
  ...
  function folder_header() {
	...
	$sel = mysql_query("SELECT display_name FROM wp_users WHERE ID = '$title'");
			while($data = mysql_fetch_assoc($sel)){			
				$title = sprintf( __( '%s&rsquo;s Album','lazyest-gallery'), $data['display_name']);
			}
			if (empty($title)) $title =  sprintf( __( '%s&rsquo;s Album','lazyest-gallery'), lg_html( $this->caption() )  );
    $thumbs_folder_header =  sprintf( '<div class="folder_caption"><h3>%s</h3></div>%s',
      apply_filters( 'lazyest_folder_caption', $title, $this ),
  ...
  function show_slide( $filename ) {
    ...
    <?php if ( 'nothing' != $lg_gallery->get_option( 'on_slide_click' ) ) : ?>  	    
      </a>         
    <?php
		endif;
		if ( in_array( $lg_gallery->get_option( 'on_slide_click' ), array( 'lightbox', 'thickbox' ) ) ) { // add links for lightbox
    ...
    if ( 1 < count( $this->list ) ){
      ...
      <?php if ( ( 'TRUE' == $lg_gallery->get_option( 'enable_slide_show' ) )  && $slideshowbutton_shortcode ) : ?>
      <a href="<?php echo add_query_arg( 'lg_show', 'true', $this->uri() ); ?>" class="lg_slideshow_button"><?php echo __( 'Slide Show', 'lazyest-gallery' ); ?></a>
		  <?php
		  endif;
      ...
      ?>
        <br style="clear: both;"/>
  		</div> 	<?php /*<div class="fb-comments" data-href="<?php echo $image->src(); ?>" data-numposts="5" ></div>*/?>  
      <?php
		}
