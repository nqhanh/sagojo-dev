...
class LazyestImage {
 ...
 /**
  ...
  /**
   * LazyestImage::comment()
   * Returns the comment for the image element
   * @example <?php $comment = $image->comment(); ?>
   * @return string
   */
  function comment() {
  	...
  	$sel = mysql_query("SELECT comment FROM wp_kaisha_camera WHERE image = '$this->image'");
  ...
  /**
   * LazyestImage::name()
   * Returns the image name for the image element
   * @example <?php $filename = $image->name(); ?>
   * @return string
   */
  function name() {
  	global $lg_gallery;
  ...
  function caption() {
    global $lg_gallery;
    $caption = ( '' != $this->caption ) ? $this->caption : str_replace( '_', ' ', esc_html( substr( utf8_encode( $this->image ), 0, -4 ) ) );
    $caption =  ( 'TRUE' == $lg_gallery->get_option( 'enable_captions' ) ) ? $caption : esc_html( utf8_encode( $this->image ) ); 
  ...
  function resize( $width, $height ) {
    ...
    $this->resized = apply_filters( 'lazyest_imageresized', $this->resized, $width, $height, $this );
		               
    return true; 
  }
  
  /**
   * LazyestImage::reset_orientation()
   * rotate image if exif orientation is set
   * 
   * @return void
   */
  function reset_orientation() {  	
  	if ( empty( $this->resized ) || ( ! function_exists( 'exif_read_data' ) ) )
  		return;
  		
		if ( $exif = @exif_read_data( $this->original() ) ) {
			$orientation = isset ( $exif['Orientation'] ) ? $exif['Orientation'] : false;
			if ( $orientation ) {
				switch( $orientation ) {
					case 3: // 180 rotate left				
						$this->resized = imagerotate($this->resized, 180, -1);				
						break;				
					case 6: // 90 rotate right				
						$this->resized = imagerotate($this->resized, -90, -1);				
						break;				
...
class LazyestSlide extends LazyestImage {  
  ...
  function src() {
    ...
    if ( 'TRUE' == $lg_gallery->get_option( 'enable_slides_cache' ) ) {
      ...
      if ( file_exists( $slidefile ) ) { // a slide has been cached
        return  apply_filters( 'lazyest_gallery_slide_src', $lg_gallery->address . lg_nice_link( $this->folder->curdir . $lg_gallery->get_option( 'slide_folder' ) . $this->image ) );
  ...
  function cache() {
    ...
    if ( isset( $lg_gallery ) && ( $this->image != '' ) ) {
      if ( 'TRUE' == $lg_gallery->get_option( 'enable_slides_cache' ) ) {
        $slide_dir = trailingslashit( dirname( $this->loc() ) );
  ...
  function loc() {
    global $lg_gallery;
		$root = defined( 'LAZYEST_GALLERY_CACHE_ROOT' ) ? LAZYEST_GALLERY_CACHE_ROOT : $lg_gallery->root;     
    return apply_filters( 'lazyest_gallery_slide_location', $root . $this->folder->curdir . $lg_gallery->get_option( 'slide_folder' ) . $this->image );
  }
...
class LazyestThumb extends LazyestImage {
  ...
  function cache() {
    ...
    if ( isset( $lg_gallery ) && ( $this->image != '' ) ) {
      if ( 'TRUE' == $lg_gallery->get_option( 'enable_cache' ) ) {
        $thumb_dir = trailingslashit( dirname( $this->loc() ) );
		    if ( ! file_exists( $this->loc() ) ) {
  ...
  function loc() {
    global $lg_gallery;
		$root = defined( 'LAZYEST_GALLERY_CACHE_ROOT' ) ? LAZYEST_GALLERY_CACHE_ROOT : $lg_gallery->root;
    return apply_filters( 'lazyest_gallery_thumb_location', $root . $this->folder->curdir . $lg_gallery->get_option( 'thumb_folder' ) . $this->image );
  }
