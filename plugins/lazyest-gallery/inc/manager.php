...
class LazyestAdminFolder extends LazyestFolder {
  ...
  function editor_toolbar() {
  	...
  	<div id="editor-toolbar" class="wp-editor-tools">
	  	<div class="hide-if-no-js wp-media-buttons" id="wp-media-buttons">
				<a title="<?php esc_html_e( 'Add an Image', 'lazyest-gallery' ) ?>" class="thickbox add_image button insert-media add_media" id="content-add_image" href="<?php echo $action_url ?>">
					<img onclick="return false;" alt="<?php esc_html_e( 'Add Image(s)', 'lazyest-gallery' ) ?>" src="<?php echo admin_url( 'images/media-button-image.gif' ); ?>" /> <?php echo esc_html__( 'Upload Image(s)', 'lazyest-gallery' ); ?>
				</a>
			</div>
		</div>
		<?php
  ...
  function breadcrumbtrail() {    
    ...
    for ( $i = 0; $i < count( $levels ) - 1; $i++ ) {
    	...
    	echo '<a title="' . esc_attr__( 'Manage level up', 'lazyest-gallery') . '" href="admin.php?page=lazyest-filemanager&amp;folder=' . lg_nice_link( $tlevel ) . '" >' . esc_html( utf8_encode( $levels[$i] ) ) . '</a> / ';
    }
    $thisdir = esc_html( utf8_encode( $levels[$i] ) );
   ...
   */
  function titlediv() {
    $titlelabel = sprintf( esc_html__( 'Enter caption for folder &#145;%s&#146; here', 'lazyest-gallery' ), utf8_encode( $this->dirname() ) );
  	...
  	if ( $flash ) { 
	    ...
	    <script type="text/javascript">
	      ...
	      SWFUpload.onload = function() {
	        var settings = {
	        	button_text: '<span class="select-upload"><?php esc_html_e('Select Files'); ?></span>',
	        	...
	        	button_image_url: '<?php echo $lg_gallery->plugin_url . '/images/upload.png'; ?>',
