...
class LazyestGallery {
  ...
  function _resolve_href ( $base, $href ) {   
    ...
    return ( (array_key_exists('scheme', $base_parsed) ) ? $base_parsed['scheme'] . '://' . $base_parsed['host'] : "" ) . $port . "/" . implode("/", $parts); 
  } 
  ...
  function init() {
    ...
    $gallery_folder = $this->get_option( 'gallery_folder' );
		...
		$root = str_replace( '\\', '/', trailingslashit( $this->get_absolute_path( ABSPATH . $gallery_folder ) ) );		
		$this->root = defined( 'LAZYEST_GALLERY_ROOT' ) ? LAZYEST_GALLERY_ROOT : $root;
		
   	$address = trailingslashit( $this->_resolve_href( trailingslashit( get_option( 'siteurl' ) ), $gallery_folder ) ) ;
		$this->address = defined( 'LAZYEST_GALLERY_ADDRESS' ) ? LAZYEST_GALLERY_ADDRESS : $address;			
  ...
  function is_dangerous( $directory ) {
  	// constant for root folder has been set by user 
  	if ( defined( 'LAZYEST_GALLERY_ROOT' ) )
  		return false;
		...
		$subdirs = $this->_build_folders_array( $directory );
		if ( ! empty( $subdirs ) ) {
  ...
  function check_safety( $options ) {
  	if ( defined( 'LAZYEST_GALLERY_ROOT' ) )
  		return $options;
  	$tmp_root = $this->get_absolute_path( ABSPATH . $options['gallery_folder'] );
    if ( $this->is_dangerous( $tmp_root ) ) {	
   ...
   * Check if the gallery root directory is set and if it exists
   * @return
   */
  function valid() { 	  
  ...
  function _default_dir() {    
    ...
    $relative = $this->get_relative_path( $abspath, $contentdir ) . '/';
    return apply_filters( 'lazyest_gallery_directory', $relative . 'lg-gallery/' );
  }
  ...
  function init_commentor() {  
	...
	/**
	 * LazyestGallery::current_url()
	 * Returns currently requested url
	 * 
	 * @since 1.1.15
	 * @return sring current url
	 */
...
require_once( $lg_plugin_dir . '/inc/version.php' );   
require_once( $lg_plugin_dir . '/inc/image.php' );    
require_once( $lg_plugin_dir . '/inc/folder.php' ); 
require_once( $lg_plugin_dir . '/inc/functions.php' );
require_once( $lg_plugin_dir . '/inc/widgets.php' );
