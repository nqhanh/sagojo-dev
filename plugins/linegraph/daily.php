...
echo "<div style=\"background-color:#e2e2e2;padding:5px 10px 5px;\">Daily job seekers</div><div style=\"float:left;width:10%;\"><table>";
for ($i=15;$i>=0;$i--) {
$sql3 = "SELECT created_at,COUNT(*) as Num FROM wpjb_resume WHERE DATEDIFF(NOW(),created_at) = $i ORDER BY YEAR(`created_at`),MONTH(`created_at`)";
...
echo "<script type=\"text/javascript\" src=\"https://www.google.com/jsapi\"></script>";
    echo "<script type=\"text/javascript\">";
      ...
      echo "}";
    echo "</script>";
   echo " <div id=\"chart_div2\" style=\"width: 900px; height: 250px;\"></div>";
...
$res_arr_values2 = array();
echo "<div style=\"clear:both;\"></div>";
echo "<div style=\"background-color:#e2e2e2;padding:5px 10px 5px;\">Daily active resumes</div><div style=\"float:left;width:10%;\"><table>";
for ($i=15;$i>=0;$i--) {
$sql3 = "SELECT created_at,COUNT(*) as Num FROM wpjb_resume WHERE is_active = 1 AND DATEDIFF(NOW(),created_at) = $i ORDER BY YEAR(`created_at`),MONTH(`created_at`)";
$query3 = mysql_query($sql3);
echo "<tr><td style=\"border:1px solid #fff;background-color:#e2e2e2;\">".date("d/m/Y", time()- 60 * 60 * 24 * $i)."</td><td style=\"border:1px solid #fff;background-color:#e2e2e2;text-align:right;\">";
while($row3 = mysql_fetch_array($query3)){
	echo $row3['Num']."</td></tr>";
...
echo "</table></div><div style=\"float:right;\">";
    echo "<script type=\"text/javascript\">";
      ...
      echo "}";
    echo "</script>";
   echo " <div id=\"chart_div3\" style=\"width: 900px; height: 250px;\"></div>";
echo "</div>";
