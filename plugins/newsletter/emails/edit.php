...
if ($controls->is_action('test')) {
    ...
    if (count($users) == 0) {
        $controls->errors = 'There are no test subscribers. Read more about test subscribers <a href="http://www.sagojo.com/plugins/newsletter/subscribers-module#test" target="_blank">here</a>.';
    } else {
        Newsletter::instance()->send(Newsletter::instance()->get_email($email_id), $users);
        $controls->messages .= 'Test emails sent to ' . count($users) . ' test subscribers. Read more about test subscribers <a href="http://www.sagojo.com/plugins/newsletter/subscribers-module#test" target="_blank">here</a>.';
    }
}
...
<div class="wrap">
    ...
    <form method="post" action="" id="newsletter-form">
        ...
        <p class="submit">
            ...
            {
                if ($email['status'] != 'sending' && $email['status'] != 'sent') $controls->button_confirm('test', 'Save and test', 'Save and send test emails to test addresses?');
            }
            ?>
            <?php if ($email['status'] == 'new') $controls->button_confirm('send', 'Send', 'Start a emails real delivery?'); ?>
            ...
            <?php
            if(current_user_can('moderate_comments')){
                $controls->button_confirm('editor', 'Save and switch to ' . ($email['editor'] == 0 ? 'HTML source' : 'visual') . ' editor', 'Sure?'); 
            }
             ?>
        </p>
        ...
        <div id="tabs">
            <ul>
                ...
                <li style="display:none;"><a href="#tabs-b">Message (textual)</a></li>
            ...
            <div id="tabs-c">
                <table class="form-table">
                    ...
                    <tr valign="top">
                        <th>Customer</th>
                        <td>
                            <?php $controls->checkboxes_group('sex', array('e'=>'Employer', 'j'=>'Jobseeker', 'n'=>'Partner')); ?>                            
                        </td>
                    </tr>
