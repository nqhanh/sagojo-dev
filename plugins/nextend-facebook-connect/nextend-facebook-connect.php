...
function new_fb_login_action() {
  ...
  if ($user && is_user_logged_in() && new_fb_is_user_connected()) {
          ...
          if ($ID == false) { // Real register
            ...
            if (!is_wp_error($ID)) {
	         ...
	         $object = new Wpjb_Model_Resume();
                ...
                $object->firstname = $user_profile['first_name'];
				$object->lastname = $user_profile['last_name'];
				$gender = $user_profile['gender'];
				$object->gender = ($gender == 'male') ? '1' : '0';
				$namsinh =  new DateTime($user_profile['birthday']);
				$object->namsinh = date_format($namsinh,'d/m/Y');
    ...
    $scope = apply_filters('nextend_fb_scope', 'email,user_birthday');
