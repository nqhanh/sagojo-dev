...
class Sidebar_Login_Widget extends WP_Widget {
    ...
    public function define_options() {
		...
		$this->options       = array(
			...
			'logged_in_title'  => array(
				...
				'default'         => __( '[:en]Welcome %username%[:vi]Chào %username%[:ja]こんにちは! %username%' ,'sidebar_login' ),
				'type'            => 'text'
			),
			'logged_in_text'  => array(
				'label'           => __( 'Logged-in text', 'sidebar_login' ),
				'default'         => __( 'Member since: %userdate%' ,'sidebar_login' ),
				'type'            => 'text'
			),
			'logged_in_links'  => array(
				...
				'default'         => "Profile | %admin_url%/profile.php\nLogout | %logout_url%",
				'type'            => 'textarea'
			),
			'show_avatar'  => array(
    ...
    public function replace_tags( $text ) {
	    ...
	    if ( $this->user ) {
			...
			$text = str_replace(
		    	array( '%userdate%' ),
		    	array( date(get_option('date_format'), strtotime($this->user->user_registered))),
		    	$text
		    );			
	    }
    ...
    public function show_links( $show = 'logged_in', $links = array() ) {
			...
			if ( get_option('users_can_register') && ! empty( $this->instance['show_register_link'] ) && $this->instance['show_register_link'] == 1 ) {
		    	...
		    	if ( ! is_multisite() ) {
		    		...
		    		$links['register'] = array(
		    			'text' => __( 'Create new account', 'sidebar_login' ),
		...
		if ( ! empty( $links ) && is_array( $links ) && sizeof( $links > 0 ) ) {
			echo '<ul class="pagenav sidebar_login_links " id="dung_rui">';
			$i=0;
			
			foreach ( $links as $id => $link ){
				if ($i==0){
					echo '<li class="' . esc_attr( $id ) . '-link" id="chuan"><a href="' . esc_url( $this->replace_tags( $link['href'] ) ) . '">' . wp_kses_post( $this->replace_tags( $link['text'] ) ) . '</a></li>';
					$i++;
				}
				else {
					//echo '<li class="' . esc_attr( $id ) . '-link" id="chuan"><span style="color:#F15A24">Not registry? </span><a href="wp-login.php?action=register">' . wp_kses_post( $this->replace_tags( $link['text'] ) ) . '</a></li>';
					echo '<li class="' . esc_attr( $id ) . '-link" id="chuan"><a href="' . esc_url( $this->replace_tags( $link['href'] ) ) . '">' . wp_kses_post( $this->replace_tags( $link['text'] ) ) . '</a></li>';
    ...
    public function widget( $args, $instance ) {
		...
		$defaults = array(
			'logged_in_title'  => ! empty( $instance['logged_in_title'] ) ? $instance['logged_in_title'] : __( '[:en]Welcome %username%[:vi]Chào %username%[:ja]こんにちは! %username%', 'sidebar_login' ),
			'logged_out_title' => ! empty( $instance['logged_out_title'] ) ? $instance['logged_out_title'] : __( '[:en]<span id="font-color">You can login by your account on <span class="app-sagojo"><a href="?page_id=100"> sagojo apps.</a></span></span></div>[:vi]<span id="font-color">Bạn có thể đăng nhập bằng tài khoản đã đăng ký trên các <span class="app-sagojo"><a href="?page_id=100"> ứng dụng của sagojo.</a></span></span></div>[:ja]<span id="font-color"><span class="app-sagojo"><a href="?page_id=100">sagojo</a></span>のスマホアプリでご登録のアカウントでログインできます。</span></div>', 'sidebar_login' ),
			'show_avatar'      => isset( $instance['show_avatar'] ) ? $instance['show_avatar'] : 1,
			'logged_in_text'  => ! empty( $instance['logged_in_text'] ) ? $instance['logged_in_text'] : __( 'Member since: %userdate%', 'sidebar_login' ),
		...
		if ( is_user_logged_in() ) {
			...
			$logged_in_text = $this->replace_tags( apply_filters( 'sidebar_login_widget_logged_in_text', $logged_in_text ) );
			if ( $logged_in_text )
				echo $logged_in_text;
		...
		// Logged out user
			...
			$login_form_args = apply_filters( 'sidebar_login_widget_form_args', array(
		        ...
		        'label_username' 	=> __( '[:en]Username or E-mail[:vi]Tên đăng nhập hoặc E-mail[:ja]ユーザ名 又は Eメール', 'sidebar_login' ),
		        'label_password' 	=> __( '[:en]Password[:vi]Mật khẩu[:ja]パスワード ', 'sidebar_login' ),
		        'label_remember' 	=> __( '[:en]Remember Me[:vi]Lưu mật khẩu[:ja]ログイン状態を保持', 'sidebar_login' ),
		        'label_log_in' 		=> __( '[:en]Login[:vi]Đăng nhập[:ja]ログイン&rarr;', 'sidebar_login' ),
		        'remember' 			=> true,
		        'value_remember' 	=> true
