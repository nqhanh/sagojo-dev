...
if ( ! class_exists( 'ITThickbox' ) ) {
	class ITThickbox {
		function render_thickbox( $content_func ) {
			$plugin_path = dirname( __FILE__ );
			$plugin_relative_path = ltrim( str_replace( '\\', '/', str_replace( rtrim( ABSPATH, '\\\/' ), '', $plugin_path ) ), '\\\/' );
			$plugin_url = get_option( 'siteurl' ) . '/' . $plugin_relative_path;
...
<?php
		...
		function delete_parent_table_row( $entry_id, $table_id, $row_class_prefix = 'entry-' ) {
...
<?php
		...
		function add_parent_table_row( $entry_id, $entry_description, $entry, $table_id, $description_link_title, $row_class_prefix = 'entry-', $insert_position = 'alpha', $enable_highlight = true, $highlight_color = '#66FF66' ) {
