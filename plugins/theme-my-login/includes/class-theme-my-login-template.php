...
class Theme_My_Login_Template extends Theme_My_Login_Abstract {
	...
	public static function get_action_template_message( $action = '' ) {
		switch ( $action ) {
			case 'register':
				...
				break;
			case 'lostpassword':
				$message = __( '[:en]Please enter your username or email address.You will receive a link to create a new password via email.[:vi]Vui lòng nhập tên người dùng hoặc địa chỉ email. Bạn sẽ nhận một đường dẫn tạo mật khẩu mới qua email.[:ja]ユーザ名またはEメールアドレスを入力して下さい。ご登録のEメールアドレス宛に新規パスワード作成のリンクをお送りします。');
				break;
			case 'resetpass':
				$message = __( '[:en]Enter your new password below.[:vi]Nhập lại mật khẩu mới vào ô bên dưới[:ja]新しいパスワードを入力して下さい。' );
				break;
			default:
				$message = '';
		}
	...
	public function the_action_template_title( $action = 'login', $before_message = '', $after_message = '' ) {
		if ( $message = self::get_action_template_message( $action ) )
			echo $before_message . $message . $after_message;
	}
