			...
			switch ( $this->request_action ) {
				...
				case 'retrievepassword' :
					...
					if ( isset( $_REQUEST['error'] ) && 'invalidkey' == $_REQUEST['error'] )
						$this->errors->add( 'invalidkey', __( '[:en]Sorry, that key does not appear to be valid.[:vi]Xin lỗi, có gì đó không hợp lệ ở đây.[:ja]Sorry, that key does not appear to be valid.' ) );
				...
				default:
					...
					if		( isset( $_GET['loggedout'] ) && true == $_GET['loggedout'] )
						$this->errors->add( 'loggedout', __( '[:en]You are now logged out.[:vi]Bạn đã đăng xuất.[:ja]ログアウトしました。.' ), 'message' );
					elseif	( isset( $_GET['registration'] ) && 'disabled' == $_GET['registration'] )
						$this->errors->add( 'registerdisabled', __( 'User registration is currently not allowed.' ) );
					elseif	( isset( $_GET['checkemail'] ) && 'confirm' == $_GET['checkemail'] )
						$this->errors->add( 'confirm', __( '[:en]Check your e-mail for the confirmation link.[:vi]Kiểm tra thư để nhận mật khẩu.[:ja]お送りした認証リンクをEメールでご確認下さい。' ), 'message' );
					elseif ( isset( $_GET['resetpass'] ) && 'complete' == $_GET['resetpass'] )
						$this->errors->add( 'password_reset', __( '[:en]Your password has been reset.[:vi]Mật khẩu của bạn đã được thay đổi[:ja]パスワードはリセットされました。' ), 'message' );
					...
					elseif	( $reauth )
						$this->errors->add( 'reauth', __( '[:en]Please log in to continue.[:vi]Vui lòng đăng nhập để kết nối.[:ja]ログインをして下さい。', 'theme-my-login' ), 'message' );
...
<?php
	...
	public static function retrieve_password() {
		...
		if ( empty( $_POST['user_login'] ) ) {
			$errors->add( 'empty_username', __( '[:en]<strong>ERROR</strong>: Enter a username or e-mail address.[:vi]<strong>Lỗi</strong>: Nhập tên người dùng hoặc địa chỉ email.[:ja]<strong>エラー</strong>: ユーザ名もしくはEメールアドレスを入力して下さい。' ) );
		} else if ( strpos( $_POST['user_login'], '@' ) ) {
			$user_data = get_user_by_email( trim( $_POST['user_login'] ) );
			if ( empty( $user_data ) )
				$errors->add( 'invalid_email', __( '[:en]<strong>ERROR</strong>: There is no user registered with that email address.[:vi]<strong>Lỗi</strong>: Không có người dùng nào đăng ký với địa chỉ email này.[:ja]エラー：そのEメールアドレスでのご登録はありません。' ) );
		} else {
		...
		if ( ! $user_data ) {
			$errors->add( 'invalidcombo', __( '[:en]<strong>ERROR</strong>: Invalid username or e-mail.[:vi]<strong>Lỗi</strong>: Người dùng hoặc e-mail không hợp lệ.[:ja]エラー:Eメールの形式が正しくありません。' ) );
			return $errors;
		}
	...
	public static function register_new_user( $user_login, $user_email ) {
		...
		if ( $sanitized_user_login == '' ) {
			$errors->add( 'empty_username', __( '[:en]<strong>ERROR</strong>: Please enter a username.[:vi]<strong>Lỗi</strong>: Vui lòng nhập tài khoản người dùng.' ) );
		} elseif ( ! validate_username( $user_login ) ) {
			$errors->add( 'invalid_username', __( '[:en]<strong>ERROR</strong>: This username is invalid because it uses illegal characters. Please enter a valid username.[:vi]<strong>Lỗi</strong>: Tên đăng nhập không hợp lệ bởi vì có những ký tự đặc biệt. Xin điền vào tên đúng.[:ja]This username is invalid because it uses illegal characters. Please enter a valid username.' ) );
