  ...
  class wp_user_avatar{
    ...
    ?>
      <?php if(class_exists('bbPress') && !is_admin()) : // Add to bbPress profile with same style ?>
        <h2 class="entry-title"><?php _e('[:en]User Avatar[:vi]Hình đại diện[:ja]アバター', 'wp-user-avatar'); ?></h2>
        <fieldset class="bbp-form">
          <legend><?php _e('[:en]User Avatar[:vi]Hình đại diện[:ja]アバター', 'wp-user-avatar'); ?></legend>
      <?php else : // Add to profile with admin style ?>
        <h3><?php _e('[:en]User Avatar[:vi]Hình đại diện[:ja]アバター', 'wp-user-avatar') ?></h3>
        <table class="form-table">
          <tr>
            <th><label for="wp_user_avatar"><?php _e('[:en]User Avatar[:vi]Hình đại diện[:ja]アバター', 'wp-user-avatar'); ?></label></th>
            <td>
      <?php endif; ?>
      ...
      <?php elseif(!current_user_can('upload_files') && !has_wp_user_avatar($current_user->ID)) : // Upload button ?>
        ...
        <button type="submit" class="button" id="upload-wp-user-avatar" name="upload-wp-user-avatar" value="<?php _e('[:en]Upload[:vi]Tải lên[:ja]アップロード'); ?>"><?php _e('[:en]Upload[:vi]Tải lên[:ja]アップロード'); ?></button>
        <p>
      ...
      <p id="wpua-preview">
        ...
        <?php _e('[:en]Original Size[:vi]Hình mặc định[:ja]オリジナルサイズ'); ?>
      </p>
      <p id="wpua-thumbnail">
        <img src="<?php echo $avatar_thumbnail; ?>" alt="" />
        <?php _e('[:en]Thumbnail[:vi]Thu nhỏ[:ja]サムネイル'); ?>
      </p>
