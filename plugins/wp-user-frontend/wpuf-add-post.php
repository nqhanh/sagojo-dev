...
class WPUF_Add_Post {
    ...
    function shortcode( $atts ) {
        ...
        extract( shortcode_atts( array('post_type' => 'freelance_post'), $atts ) );
...
<script type="text/javascript">
	$(function(){
		...
		$.ajax({
			...
			<fieldset id="wpuf_fieldset_post_form">
				<legend><?php _e( 'Project Information', 'wpuf' ); ?></legend>
                    ...
                    <ul class="wpuf-post-form">
                        ...
                        <li>
                            <label for="new-post-title">
                                <?php echo wpuf_get_option( 'title_label', 'wpuf_labels', 'Project Title'  ); ?> <span class="required">*</span>
                            </label>
                        ...
                        <?php if ( wpuf_get_option( 'allow_cats', 'wpuf_frontend_posting', 'on' ) == 'on' ) { ?>
                            <li>
                                <label for="new-post-cat">
                                    <?php echo wpuf_get_option( 'cat_label', 'wpuf_labels', 'Category' ); ?>
                                </label>

                                <span class="category-wrap" style="float:left;">
                                    <span id="lvl0">
                                        <?php
                                        ...
                                        if ( $cat_type == 'normal' ) {
											wp_dropdown_freelance_category('show_count=0&selected=-1&hierarchical=1&depth=1&hide_empty=0&exclude=1&show_option_none=' . __( 'Select a category of work (Optional)', 'wpuf' ) . '&name=main_cat');

											//wp_dropdown_freelance_category('show_count=0&selected=77&hierarchical=1&depth=1&hide_empty=0&class=cat requiredField&exclude=1&name=main_cat');
                                           // wp_dropdown_freelance_category( 'show_option_none=' . __( '-- Select --', 'wpuf' ) . '&hierarchical=1&hide_empty=0&orderby=name&name=category&id=cat&show_count=0&title_li=&use_desc_for_title=1&class=cat requiredField&exclude=' . $exclude );
                                            ...
                                            wp_dropdown_freelance_category( 'show_option_none=' . __( '-- Select --', 'wpuf' ) . '&hierarchical=1&hide_empty=0&orderby=name&name=category&id=cat-ajax&show_count=0&title_li=&use_desc_for_title=1&class=cat requiredField&depth=1&exclude=' . $exclude );
                                        } else {
                                            wpuf_category_checklist(0, false, 'category', $exclude);
                                        }
                                        ?>
                                        <select name="category" id="category" disabled="disabled" class="requiredField"><option value="-1" selected="selected"><?php _e('Select a job (Optional)', 'wpuf');?></option></select>
                        ...
                        <li>
                            <label for="new-post-desc">
                                <?php echo wpuf_get_option( 'desc_label', 'wpuf_labels', 'Project Content' ); ?> <span class="required">*</span>
                            </label>
						...
						$this->deadline_date_form();
						//$this->tag_suggest_form();
                        wpuf_build_custom_field_form( 'tag' );

                        if ( wpuf_get_option( 'allow_tags', 'wpuf_frontend_posting', 'on' ) == 'on' ) {

                            /*Tag suggest by Hanh*/								
                            	$tags = get_freelance_tags($args);
                            	$html=array();
                            	foreach ( $tags as $tag ) {
                            		array_push($html,"'".$tag->name."'");
                            	}
                            	$array_tags = implode(", ",$html);
                            	?>
                            	<script src="<?php echo plugins_url('js/jquery-1.9.0.js', __FILE__ );?>"></script>
                            	<script src="<?php echo plugins_url('js/jquery-ui-1.10.0.custom.js', __FILE__ );?>"></script>
                            	<script src="<?php echo plugins_url('js/jquery.taghandler.js', __FILE__ );?>"></script>	
                            	
								<script type="text/javascript">
									var tag = $.noConflict(true);
								</script>
							    <link type="text/css" href="<?php echo plugins_url('css/jquery.taghandler.min.css', __FILE__ );?>" rel="stylesheet">
							    <link type="text/css" href="<?php echo plugins_url('css/jquery-ui.css', __FILE__ );?>" rel="stylesheet">
							    <link type="text/css" href="<?php echo plugins_url('css/prettify.css', __FILE__ );?>" rel="stylesheet">
								    ...
								    tag(function(){
								        	...
								        	tag(".methodExample").tagHandler({
								                ...
								                autocomplete: true,
												maxTags:5,
												afterAdd: function() {document.getElementById("mytag").value = (tag("#example_get_tags").tagHandler("getTags").join(","));},
												afterDelete: function() {document.getElementById("mytag").value = (tag("#example_get_tags").tagHandler("getTags").join(","));}
								            });
							...
							<li>
                            <?php _e('Enter some skills that relate to the project:', 'wpuf' );?></li>
                            <li><label>&nbsp;</label>
                             <ul id="example_get_tags" class="methodExample"></ul>
								<input type="hidden" id="mytag" name="wpuf_post_tags">
							 </li>
                            <?php   
	...
	/**
     * Prints post budget on the form
     *
     * @return 
     */
    function budget_form() {       
        ...
        <li>
			...
			<input type="text" name="budget" id="budget" placeholder="<?php _e( 'input numeric (ex: 10000)...', 'wpuf' ); ?>" />
			<select name="currency"><option name=VND value=VND>VND</option><option name=USD value=USD>USD</option></select>	
            <div class="clear"></div>
        </li>
        <?php
    }
	/**
     * Prints post deadline on the form
     *
     * @return 
     */
    function deadline_date_form() {       
        ?>
        <li >
            <label for="timestamp-wrap">
                <?php _e( 'Deadline', 'wpuf' ); ?><span class="required">*</span>
            </label>				
			<script src="<?php echo plugins_url('js/jquery-1.js', __FILE__ );?>"></script>
			<script src="<?php echo plugins_url('js/kendo.js', __FILE__ );?>"></script>
			<div id="to-do">
                    <span class="k-widget k-datetimepicker k-header" style="width: 200px;">
						<span class="k-picker-wrap k-state-default">
							
								<input value="9/19/2013 5:00 AM" aria-activedescendant="datetimepicker_option_selected" aria-label="Current focused date is 9/19/2013 5:00:00 AM" aria-readonly="true" aria-disabled="true" aria-expanded="true" aria-haspopup="true" role="textbox" class="k-input" data-role="datetimepicker" name="deadline" id="deadline" style="width: 100%;" type="text" />
						
							<span unselectable="on" class="k-select">
									<span aria-controls="datetimepicker_dateview" role="button" unselectable="on" class="k-icon k-i-calendar">select</span>
									<span aria-controls="datetimepicker_timeview" role="button" unselectable="on" class="k-icon k-i-clock">select</span>
							</span>
						</span>
					</span>
			</div>
			...
			<!--<input type="text" name="deadline" id="deadline" class="datetimepicker"/>-->
            <div class="clear"></div>
        </li>
        <?php 
    } 
    ...
    function submit_post() {
        ...
        if ( wpuf_get_option( 'allow_cats', 'wpuf_frontend_posting', 'on' ) == 'on' ) {
            ...
            if ( !isset( $_POST['category'] ) || $_POST['category'] == -1 ){
				$_POST['category'] = array(wpuf_get_option( 'default_cat', 'wpuf_frontend_posting' ));
        ...
        if ( isset( $_POST['most_post_tags'] ) ) {
			foreach($_POST["most_post_tags"] as $tag){
        		array_push($tags, $tag);
        	}
        }
		...
		wp_set_post_terms( $post_id, $tags, 'freelance_tag' );
		wp_set_post_terms($post_id, $post_category, 'freelance_category');
        ...
        if ( $post_id ) {
			...
			//Set Post deadline date if has any
			if ( empty( $_POST['deadline'] ) ) {
            $errors[] = __( 'Empty deadline', 'wpuf' );
			} else {
                $deadl = $_POST['deadline'];
                /*$val = time() + $deadl;*/
				$val = $deadl;
                add_post_meta( $post_id, 'cf_deadline', $val, true );
            }
			
			//Set Budget if has any
            if ( !empty( $_POST['budget'] )) {
                $val = $_POST['budget'];
                $cur = $_POST['currency'];
                add_post_meta( $post_id, 'cf_budget', $val, true );
                add_post_meta( $post_id, 'cf_currency', $cur, true );                
            }
            else {	add_post_meta( $post_id, 'cf_budget', '0', true );
					add_post_meta( $post_id, 'cf_currency', $cur, true );
				}
            //plugin API to extend the functionality
