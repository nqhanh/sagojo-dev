...
class Wpjb_Form_AddJob extends Wpjb_Form_AbstractJob
{
    ...
    protected function _getListingArr()
    {
        ...
        $query->select("t.*")->from("Wpjb_Model_Listing t")->where("is_active=1");
    ...
    public function init()
    {
        ...
        $e->setLabel(__("Payment code", WPJB_DOMAIN));
		$e->setHint(__("Input your payment code", WPJB_DOMAIN));
        ...
        $e->setRequired(true);
        $e->setLabel(__("Job pack", WPJB_DOMAIN));
        ...
        foreach($this->_getListingArr() as $listing) {
			...
			if($listing->title=="Free") $listingtitle = __("Free", WPJB_DOMAIN);
			else if($listing->title=="Standard") $listingtitle = __("Standard", WPJB_DOMAIN);
			else if($listing->title=="Professional") $listingtitle = __("Professional", WPJB_DOMAIN);
			else if($listing->title=="Enterprise") $listingtitle = __("Enterprise", WPJB_DOMAIN);
            /*bo--$e->addOption($listing->id, $listing->id, str_replace($find, $repl, $tpl));*/
			$e->addOption($listing->id, $listing->id, $listingtitle);
        }
        /*bo--$e->setRenderer("wpjb_form_helper_listing");*/
		$e->setHint(__("Choose job pack - <a href='http://www.sagojo.com/job-pack' target=_blank>View job pack detail</a>", WPJB_DOMAIN));
        $this->addElement($e, "coupon");					
