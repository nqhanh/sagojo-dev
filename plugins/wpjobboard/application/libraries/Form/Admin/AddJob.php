...
class Wpjb_Form_Admin_AddJob extends Wpjb_Form_AbstractJob
{
    ...
    public function init()
    {
        ...
        if($this->getObject()->getId() && $this->isAdminMode()) {
            ...
            $e->setLabel(__("Color highlighted", WPJB_DOMAIN));
            $e->setValue($this->getObject()->is_featured);
            $e->addOption(1, 1, __("Yes", WPJB_DOMAIN));
            $e->addFilter(new Daq_Filter_Float());
            $this->addElement($e, "other");
			
			$e = new Daq_Form_Element("is_new", Daq_Form_Element::TYPE_CHECKBOX);
            $e->setLabel(__("Listed on the homepage", WPJB_DOMAIN));
            $e->setValue($this->getObject()->is_new);
            $e->addOption(1, 1, __("Yes", WPJB_DOMAIN));
            $e->addFilter(new Daq_Filter_Float());
            $this->addElement($e, "other");
			
			$e = new Daq_Form_Element("is_hot", Daq_Form_Element::TYPE_CHECKBOX);
            $e->setLabel(__("Featured Job", WPJB_DOMAIN));
            $e->setValue($this->getObject()->is_hot);
            $e->addOption(1, 1, __("Yes", WPJB_DOMAIN));
            $e->addFilter(new Daq_Filter_Float());
            $this->addElement($e, "other");
			
			$e = new Daq_Form_Element("is_top", Daq_Form_Element::TYPE_CHECKBOX);
            $e->setLabel(__("Priority Search", WPJB_DOMAIN));
            $e->setValue($this->getObject()->is_top);
            $e->addOption(1, 1, __("Yes", WPJB_DOMAIN));
            $e->addFilter(new Daq_Filter_Float());
            $this->addElement($e, "other");
			...
			$e = new Daq_Form_Element("job_f_visible");
            $e->setLabel(__("Feature's visible", WPJB_DOMAIN));
            $e->setValue($this->getObject()->job_f_visible);
            $e->setHint(__("Number of days which feature will be visible (Overwrites listing type settings)", WPJB_DOMAIN));
            $e->addFilter(new Daq_Filter_Int());
            $e->addValidator(new Daq_Validate_Int(0));
            $this->addElement($e, "other");
        }
        ...
        if(!$this->getObject()->getId()) {
            ...
            foreach($this->_getListingArr() as $listing) {
                $e->addOption($listing->id, $listing->id, __($listing->title));
            }
            $this->addElement($e, "other");
        }
    ...
    public function isValid($values)
    {
		...
		if($this->hasElement("job_f_visible")) {
            // dokonczyc tutaj poprawne zapisywanie daty
            $days = $values["job_f_visible"];
            $now = $this->getObject()->job_created_at;

            $e = new Daq_Form_Element("feature_expires_at");
            if($days == 0) {
                $fexpiresAt = "9999-12-31 23:59:59";
            } else {
                $fexpiresAt = strtotime("$now +".$days." days");
                $fexpiresAt = date("Y-m-d H:i:s", $fexpiresAt);
            }
            $e->setValue($fexpiresAt);
            $this->addElement($e);
        }
        ...
        if($isValid && $this->getObject()->getId() < 1) {
			...
			$e = new Daq_Form_Element("feature_expires_at");
            if($listing->f_visible == 0) {
                $fexpiresAt = "9999-12-31 23:59:59";
            } else {
                $fexpiresAt = strtotime("now +".$listing->f_visible." days");
                $fexpiresAt = date("Y-m-d H:i:s", $fexpiresAt);
            }
            $e->setValue($fexpiresAt);
            $this->addElement($e);
			...
			$e = new Daq_Form_Element("is_new");
            $e->setValue($listing->is_new);
            $this->addElement($e);
			
			$e = new Daq_Form_Element("is_hot");
            $e->setValue($listing->is_hot);
            $this->addElement($e);
			
			$e = new Daq_Form_Element("is_top");
            $e->setValue($listing->is_top);
            $this->addElement($e);
