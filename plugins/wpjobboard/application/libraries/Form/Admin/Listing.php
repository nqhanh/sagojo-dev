...
class Wpjb_Form_Admin_Listing extends Daq_Form_ObjectAbstract
{
    ...
    public function init()
    {
		...
		$e = new Daq_Form_Element("f_visible");
        $e->setValue($this->_object->f_visible);
        $e->setLabel(__("Feature's visible", WPJB_DOMAIN));
        $e->setHint(__("Number of days which feature will be visible. (0 = forever)", WPJB_DOMAIN));
        $e->addFilter(new Daq_Filter_Int());
        $e->addValidator(new Daq_Validate_Int(0));
        $this->addElement($e);
        ...
        $e->setLabel(__("Is Highlight", WPJB_DOMAIN));
        $e->setHint(__("Displayed in highlight box.", WPJB_DOMAIN));
        $e->addFilter(new Daq_Filter_Int());
        $e->addOption(1, 1, __("Yes", WPJB_DOMAIN));
        $this->addElement($e);
		
		$e = new Daq_Form_Element("is_new", Daq_Form_Element::TYPE_CHECKBOX);
        $e->setValue($this->_object->is_new);
        $e->setLabel(__("New job position", WPJB_DOMAIN));
        $e->setHint(__("Displayed at the new job box.", WPJB_DOMAIN));
        $e->addFilter(new Daq_Filter_Int());
        $e->addOption(1, 1, __("Yes", WPJB_DOMAIN));
        $this->addElement($e);
		
		$e = new Daq_Form_Element("is_hot", Daq_Form_Element::TYPE_CHECKBOX);
        $e->setValue($this->_object->is_hot);
        $e->setLabel(__("Is Featured", WPJB_DOMAIN));
        $e->setHint(__("Displayed at the featured job box", WPJB_DOMAIN));
        $e->addFilter(new Daq_Filter_Int());
        $e->addOption(1, 1, __("Yes", WPJB_DOMAIN));
        $this->addElement($e);
		
		$e = new Daq_Form_Element("is_top", Daq_Form_Element::TYPE_CHECKBOX);
        $e->setValue($this->_object->is_top);
        $e->setLabel(__("Display on top", WPJB_DOMAIN));
        $e->setHint(__("In short hotted listings are listings that are displayed before normal listings (so they usually cost more).", WPJB_DOMAIN));
        $e->addFilter(new Daq_Filter_Int());
        $e->addOption(1, 1, __("Yes", WPJB_DOMAIN));
        $this->addElement($e);
