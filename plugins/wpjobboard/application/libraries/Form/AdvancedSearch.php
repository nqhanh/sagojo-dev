...
class Wpjb_Form_AdvancedSearch extends Daq_Form_Abstract
{
    public function init()
    {
        ...
        $e->setValue(__("Enter job title, position,...", WPJB_DOMAIN));
        ...
        foreach(Wpjb_Utility_Registry::getJobTypes() as $obj) {
            $e->addOption($obj->id, $obj->id, __($obj->title));
        }
		...
		$e = new Daq_Form_Element("location", Daq_Form_Element::TYPE_SELECT);
    	...
    	if(count(Wpjb_List_City::getAll()) > 0) {
            $this->addElement($e, "search");
        }
