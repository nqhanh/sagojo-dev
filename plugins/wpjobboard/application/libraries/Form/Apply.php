...
class Wpjb_Form_Apply extends Daq_Form_ObjectAbstract
{
    ...
    public function init()
    {
        ...
        $e = new Daq_Form_Element("userid", Daq_Form_Element::TYPE_HIDDEN);
        $e->setLabel(__("User ID", WPJB_DOMAIN));
        $this->addElement($e, "apply");
        
        $e = new Daq_Form_Element("resume_id_txt", Daq_Form_Element::TYPE_HIDDEN);
        $e->setLabel(__("Resume ID", WPJB_DOMAIN));
        $this->addElement($e, "apply");
        ...
        if ($resume->title==""||$resume->headline==""||$resume->experience==""||$resume->education==""){
        	$e = new Daq_Form_Element("unfilled", Daq_Form_Element::TYPE_HIDDEN);
        	$e->setLabel(__("You are unfilled resume", WPJB_DOMAIN));
        }
        
        else { if(($resume->id>0 && !$isEmp) || is_admin()) {
            $e = new Daq_Form_Element("resume_id", Daq_Form_Element::TYPE_CHECKBOX);
            $e->setLabel(__("Send my profile resume", WPJB_DOMAIN));
            $id = $resume->id;
            $e->addOption($id, $id, "<a href='".site_url()."/vi/resumes-board/my-resume' target=_blank>".__($resume->title)."</a>");
        ...
        $e->addValidator(new Daq_Validate_StringLength(1, NULL));
        //$e->addValidator(new Daq_Validate_Textarea());
        $this->addElement($e, "apply");
    ...
    {	
        ...
        $wpdb->query(
        		...
        		UPDATE wpjb_viewed
        		SET isApply = 1
        		WHERE JS_ID = $this->_userId
        		AND J_ID = $this->_jobId
        		"
        );
    ...
    protected function _saveFiles()
    {
        ...
        elseif ( $this->hasElement("resume_id_txt")){
        	$resume_id = $this->getElement("resume_id_txt")->getValue();
        }
