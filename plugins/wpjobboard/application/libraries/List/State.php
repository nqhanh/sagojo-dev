...
class Wpjb_List_State
{
    public static function getByCode($code, $country = null)
    {
        $all = self::getAll();
        if(isset($all[$code])) {
            return $all[$code];
    ...
    public static function find($pattern, $country)
    {
        $list = array();

        foreach(self::getByCountry($country) as $c) {
            if(stripos($c, $pattern) !== false) {
                $list[] = $c;
            }
        }
    ...
    public static function getAll()
    {
        ...
        $c[704] = array(
            '08'=>"[:en]Ho Chi Minh[:vi]TP Hồ Chí Minh[:ja]Ho Chi Minh",
        ...
        // add_filter to allow adding different states
        $c = apply_filters("wpjb_list_states", $c);
