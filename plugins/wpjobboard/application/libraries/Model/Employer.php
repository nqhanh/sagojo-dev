...
class Wpjb_Model_Employer extends Daq_Db_OrmAbstract
{
    ...
    {
        $path = Wpjb_List_Path::getPath("company_logo");
        $path.= "/cover_".$this->getId().".*";
    ...
    public function save()
    {
		...
		$m5 = (bool)$this->_fields["company_address"]["modified"];
        if($m1 || $m2 || $m3 || $m4 ||$m5) {
            // reset geolocation
    ...
    protected function _locate($asyncSave = false)
    {
        ...
        //$city = Wpjb_List_Country::getByCode($this->company_location);
        //$city = trim($country['name']);
		$city = $this->company_address;
        $addr = array(
            $city,
    ...
    public function locationToString()
    {
		...
		/*---chinh sua-them city combobox--*/
		$city = Wpjb_List_City::getByCode($this->company_location);
        if ($city){
        $city = trim($city['name']);
		$arr[] = $city;
		} elseif(strlen(trim($this->company_location))>0) {
            $arr[] = $this->company_location;		
        }
		
        if($this->company_state == 840 && strlen(trim($this->company_state))>0) {
