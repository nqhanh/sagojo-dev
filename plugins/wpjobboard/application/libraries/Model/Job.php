...
class Wpjb_Model_Job extends Daq_Db_OrmAbstract
{
    ...
    public function save()
    {
		...
		$m5 = (bool)$this->_fields["job_address"]["modified"];
        if($m1 || $m2 || $m3 || $m4 || $m5) {
            // reset geolocation
        ...
        if(($isNew) && ($this->job_source == 1)) {
            //Wpjb_Utility_Messanger::send(1, $this);
			if($isFree){
				Wpjb_Utility_Messanger::send(14, $this);
				Wpjb_Utility_Messanger::send(1, $this);
			}
			else {
				Wpjb_Utility_Messanger::send(13, $this);
				Wpjb_Utility_Messanger::send(16, $this);
			}
        }
    ...
    private function _approve()
    {
        ...
        try {
            if($this->job_source == 1) {
                if($this->payment_sum > 0||$this->payment_discount > 0) {
                    Wpjb_Utility_Messanger::send(4, $this);
                } else {
                    Wpjb_Utility_Messanger::send(3, $this);
                }
            }
			if(($this->job_source == 0)||($this->job_source == 2)) {
                Wpjb_Utility_Messanger::send(15, $this);               
            }
	...
	public function fexpiresAt($dateOnly = false)
    {
        if($this->job_f_visible == 0) {
            return null;
        }

        $created = strtotime($this->job_created_at." +".$this->job_f_visible." DAYS");
    ...
    public function paymentAmount()
    {
        ...
        return $curr.number_format($this->payment_sum,2);
    }
    ...
    public function paymentPaid()
    {
        ...
        $curr = Wpjb_List_Currency::getCurrencySymbol($this->payment_currency);
        return $curr.number_format($this->payment_paid,0);
    }
    ...
    public function paymentDiscount()
    {
        ...
        $curr = Wpjb_List_Currency::getCurrencySymbol($this->payment_currency);
        return $curr.number_format($this->payment_discount,2);
    }
     ...
     public function locationToString()
    {
		...
		$city = Wpjb_List_City::getByCode($this->job_location);	
        $city = trim($city['name']);
        if (strlen($city)>0){
		$arr[] = $city;
		} elseif(strlen(trim($this->job_location))>0) {
    ...
    public static function activeSelect($joined = true)
    {
        ...
        $query = $query->select("*")
        	...
        	->order("t1.is_top DESC,t1.job_created_at DESC,t1.id DESC");
	...
	public static function activeFeature($joined = true)
    {
        $query = new Daq_Db_Query();
        $query = $query->select("*")
            ->from("Wpjb_Model_Job t1")
            ->where("t1.is_active = 1")
            ->where("t1.feature_expires_at >= ?", date("Y-m-d 23:59:59"))
            /*->order("t1.is_featured DESC, t1.job_created_at DESC");*/
			//->order("t1.job_created_at DESC");
        	->order("t1.id DESC");
    ...
    protected function _locate($asyncSave = false)
    {
        $country = Wpjb_List_Country::getByCode($this->job_country);
        $country = trim($country['name']);
		//$city = Wpjb_List_Country::getByCode($this->job_location);
        //$city = trim($city['name']);
