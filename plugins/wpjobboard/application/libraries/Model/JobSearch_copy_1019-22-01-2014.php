    ...
    public static function search($params)
    {
        ...
        if($searchString && $location) {
        	if($location=='TP Hồ Chí Minh'){
        		$locarray = array("TP Hồ Chí Minh","TP Ho Chi Minh","HCM","Ho Chi Minh","TP. HCM","TP.HCM","Hồ Chí Minh");
        		$query_parts = array();
        		foreach ($locarray as $val) {
        			$query_parts[] = "'%".mysql_real_escape_string($val)."%'";
        		}
        		$string = implode(' OR t4.location LIKE ', $query_parts);
        		$select->where("t4.location LIKE ?", "$string");
        	}
        	if($location=='TP Hà Nội'){
        		$locarray = array('TP Hà Nội','TP Ha Noi','HN','Hà Nội','TP. HN','P.HN');
        		$query_parts = array();
        		foreach ($locarray as $val) {
        			$query_parts[] = "'%".mysql_real_escape_string($val)."%'";
        		}
        	...
        	if($location=='TP Đà Nẵng'){
        		$locarray = array('TP Đà Nẵng','TP Da Nang','DN','Đà Nẵng','TP. DN','TP.DN');
        		$query_parts = array();
        		foreach ($locarray as $val) {
        			$query_parts[] = "'%".mysql_real_escape_string($val)."%'";
        		}
        	...
        	else $select->where("t4.location LIKE ?", "%$location%");
        	} elseif($location) {
        		
        	if($location=='TP Hồ Chí Minh'){
        		$locarray = array("TP Hồ Chí Minh","TP Ho Chi Minh","HCM","Ho Chi Minh","TP. HCM","TP.HCM","Hồ Chí Minh");
        		//$locarray = array("TP.HCM");
        		$query_parts = array();
        		foreach ($locarray as $val) {
        			$query_parts[] = "%".mysql_real_escape_string($val)."%";
        		}
