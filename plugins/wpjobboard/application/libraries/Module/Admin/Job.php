...
class Wpjb_Module_Admin_Job extends Wpjb_Controller_Admin
{
    ...
    public function indexAction()
    {
        ...
        if($show == "active") {
            ...
            $query2->where("t1.is_active = 0");
        } elseif($show == "adminpost") {
            $query1->where("t1.job_source = 2");
			$query2->where("t1.job_source = 2");                    
		} elseif($show == "organic") {
            $query1->where("t1.job_source = 1");
			$query2->where("t1.job_source = 1");                    
		} elseif($show == "import") {
            $query1->where("t1.job_source = 0");
			$query2->where("t1.job_source = 0");
        }
		...
		$adminpost_query = new Daq_Db_Query();
        $adminpost_query = $adminpost_query->select("COUNT(*) AS c_adminpost")
            ->from("Wpjb_Model_Job t1")
            ->where("t1.job_source = 2");
		$adminpost_query = $adminpost_query->fetch();
		
		$organic_query = new Daq_Db_Query();
        $organic_query = $organic_query->select("COUNT(*) AS c_organic")
            ->from("Wpjb_Model_Job t1")
            ->where("t1.job_source = 1");
		
		$organic_summary = $organic_query->fetch();
        if(is_numeric($days)) {
            $query->where("t1.job_created_at > DATE_SUB(NOW(), INTERVAL ? DAY)", $days);
        }
        if($employer > 0) {
		...
		$stat->adminpost = $adminpost_query->c_adminpost;
		$stat->organic = $organic_summary->c_organic;
		$stat->import = $summary1->c_total - $organic_summary->c_organic - $adminpost_query->c_adminpost;
