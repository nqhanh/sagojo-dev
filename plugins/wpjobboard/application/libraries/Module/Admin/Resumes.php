...
class Wpjb_Module_Admin_Resumes extends Wpjb_Controller_Admin
{
    ...
    public function indexAction()
    {
        ...
        $query2->select()->from("Wpjb_Model_Resume t")
            ->join("t.users t2")
            ->order("t.updated_at DESC");        
        ...
        if($show == "pending") {
			...
			$query2->where("t.is_approved=?", Wpjb_Model_Resume::RESUME_PENDING);
			
        } elseif($show == "active") {
            $query->where("t.is_active = 1");
            $query2->where("t.is_active = 1");
        }
		$prep = clone $query2;
		...
		$active_query = new Daq_Db_Query();
        $active_query = $active_query->select("COUNT(*) AS c_active")
            ->from("Wpjb_Model_Resume t1")
            ->where("t1.is_active = 1");
		$active_query = $active_query->fetch();

        $this->view->show = $show;

        $stat = new stdClass();
        $stat->total = $total->fetchColumn();
		$stat->active = $active_query->c_active;
