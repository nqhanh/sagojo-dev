...
class Wpjb_Module_Frontend_Employer extends Wpjb_Controller_Frontend
{
    ...
    public function newAction()
    {
        ...
        $this->_setTitle(__("Find top talent", WPJB_DOMAIN));

        $form = new Wpjb_Form_Frontend_Register();
        $this->view->errors = array();

        if($this->isPost()) {
			/*$isbig = $form->getElement("isbig")->getValue();*/
            $isValid = $form->isValid($this->getRequest()->getAll());
            if($isValid) {
				/*$isbig = $form->getElement("isbig")->getValue();*/
                $user_first_name = $form->getElement("user_first_name")->getValue();
                $user_last_name = $form->getElement("user_last_name")->getValue();
				$user_login = $form->getElement("user_login")->getValue();
                /*$password = $form->getElement("user_password")->getValue();*/
				$password = wp_generate_password( 8, false);
                $email = $form->getElement("user_email")->getValue();
                $id = wp_create_user($user_login, $password, $email, $user_first_name, $user_last_name);
                $val = $form->getValues();
				/*$location = $form->getElement("company_location")->getValue();
				$txtlocation = $form->getElement("company_location_text")->getValue();
				if ($txtlocation=='') $com_location = $location;
				else $com_location = $txtlocation;*/

                update_usermeta($id, "is_employer", 1);
				update_usermeta($id, "is_agree", 1);
                ...
                $employer->company_country = $val["company_country"];
                $employer->company_state = $val["company_state"];
                $employer->company_zip_code = $val["company_zip_code"];
                $employer->company_location = $val["company_location"];					
                $employer->is_public = $val["is_public"];
				/*$employer->is_big = $val["isbig"];*/
                $employer->save();
                ...
                $this->view->_flash->addInfo(__("You have been registered successfully. A password will be e-mailed to you.", WPJB_DOMAIN));
    ...
    public function panelactiveAction()
    {
        ...
        $this->_setTitle(__("Active listing", WPJB_DOMAIN));
    ...
    public function panelexpiredAction()
    {
        ...
        $this->_setTitle(__("Expired listings", WPJB_DOMAIN));
    ...
    public function logodeleteAction()
    {
        ...
        $emp = Wpjb_Model_Employer::current();
        $emp->deleteImage();
        $emp->company_logo_ext = "";
        $emp->save();
        
        $this->view->_flash->addInfo(__("Logo deleted.", WPJB_DOMAIN));
        
        wp_redirect(wpjb_link_to("employer_edit"));
        exit;
    }
	...
	public function coverdeleteAction()
    {
        ...
        $this->view->_flash->addInfo(__("Cover deleted.", WPJB_DOMAIN));
    ...
    public function employereditAction()
    {
        ...
        $this->view->company = new Wpjb_Model_Employer($emp->id);
        $this->view->form = $form;
