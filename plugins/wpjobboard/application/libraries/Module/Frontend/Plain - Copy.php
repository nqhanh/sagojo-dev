...
class Wpjb_Module_Frontend_Plain extends Wpjb_Controller_Frontend
{
    ...
    public function cronAction()
    {
        ...
        $select = $query->select();
        $select->from("Wpjb_Model_Job t");
        $select->where("DATE(job_expires_at) = DATE_ADD(CURDATE(), INTERVAL 5 DAY)");
    ...
    public function feedAction()
    {
        ...
        $result = $query->execcute();
        $this->_feed($result);
    }

    protected function _feed($result) //TODO: hrtrhau
    {
        header("Content-type: application/xml");
        $rss = new DOMDocument();
        $rss->preserveWhiteSpace=true; #hntrhau
        $rss->formatOutput = true;

        $wraper = $rss->createElement("rss");
        $wraper->setAttribute("version", "2.0");
        $wraper->setAttribute('xmlns:rss', "http://www.w3.org/2005/Atom");

        $channel = $rss->createElement("channel");

        $title = $rss->createElement("title", "Tất cả ngành nghề");
        $channel->appendChild($title);
        $link = $rss->createElement("link", $this->_esc(Wpjb_Project::getInstance()->getUrl()));
        $channel->appendChild($link);
        $description = $rss->createElement("description", "Tất cả các ngành nghề");
        ...
        foreach($result as $job) {
            ...
            //$desc = $this->_substr($job->job_description); #hntrhau

            $desc="<pre><b>Tên công ty:</b> ".$job->company_name."\n <br/> ";

            $desc.="<b>Nơi làm việc:</b> ".$job->job_address."\n <br/> ";

            $desc.="<b>Mức lương</b>: Thỏa thuận"."\n <br/> ";

            $desc.="<b>Mô tả: </b><br/> "."\n <br/>";
            $desc .= $job->job_description."\n <br/>"; #hntrhau

            $desc.="<b>Yêu cầu:</b><br/> ".$job->job_required."\n <br/> ";

            $desc.="<b>Quyền lợi:</b><br/> ".$job->job_interest."\n <br/> ";

            $desc.="<b>Người liên hệ:</b> ".$job->job_contact."\n <br/> ";

            $desc.="<b>Điện thoại liên hệ:</b> ".$job->job_phone."\n <br/> ";

            $desc.="<b>Địa chỉ liên hệ:</b> ".$job->job_address."\n <br/> ";

            $desc.="<b>Yêu cầu khác:</b><br/> ".$job->contact_description."</pre>";
            //$desc.="<pre><b><a style='color: orangered; text-decoration: none; background: #0A246A; padding:5px;' href='".$link."'>Ứng tuyển</a></b></pre>";
            ...
            $item->appendChild($rss->createElement("title", "<<< VIỆC LÀM HẤP DẪN - ĐANG TUYỂN >>> <br/> ".$this->_esc($job->job_title)));
    ...
    public function trackerAction()
    {
        ...
        setcookie("wpjb[$id]", time(), time()+(3600*24*30), COOKIEPATH, COOKIE_DOMAIN, false);
