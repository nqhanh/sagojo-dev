...
class Wpjb_Module_Resumes_Index extends Wpjb_Controller_Frontend
{
    ...
    public function registerAction()
    {
        ...
        $this->_setTitle(__("For Jobseeker", WPJB_DOMAIN));
        ...
        if($this->isPost()) {
            ...
            if($isValid) {
				...
				$learnfr = $form->getElement("learn_from")->getValue();
				$birthday = $form->getElement("user_birthday")->getValue();
				$gender = $form->getElement("gender")->getValue();
                $id = wp_create_user($user_login, $password, $email, $user_first_name, $user_last_name);
				
				update_usermeta($id, "is_jobseeker", 1);
				update_usermeta($id, "is_agree", 1);
				update_usermeta($id, "learn_from", $learnfr);
				/*update_usermeta($id, "rpr_nm_sinh", $birthday);*/	
                ...
                $this->view->_flash->addInfo(__("You have been registered. A password will be e-mailed to you.", WPJB_DOMAIN));
