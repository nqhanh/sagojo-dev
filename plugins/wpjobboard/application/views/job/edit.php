...
<form action="" method="post" enctype="multipart/form-data" class="wpjb-form">
    <table class="form-table">
        ...
        <tr valign="top">
		 ...
		 <tr valign="top">
            <th scope="row"><?php _e("Featured Expires", WPJB_DOMAIN) ?></th>
            <td class="wpjb-normal-td">
                <?php if($obj->job_f_visible>0): ?>
                <?php echo date("Y-m-d", strtotime($obj->fexpiresAt()))  ?> (<?php _e("Number of days visible", WPJB_DOMAIN) ?>: <?php echo $obj->job_f_visible ?>)
                <?php else: ?>
                <i><?php _e("never", WPJB_DOMAIN) ?></i>
                <?php endif; ?>
            </td>
