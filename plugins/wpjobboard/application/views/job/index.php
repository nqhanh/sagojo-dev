...
<ul class="subsubsub">
	...
	<li><a <?php if($show == "adminpost"): ?>class="current"<?php endif; ?>href="<?php echo $this->_url->linkTo("wpjb/job", "index/show/adminpost/".$qstring) ?>"><?php _e("Posted by Admin", WPJB_DOMAIN) ?></a><span class="count">(<?php echo (int)$stat->adminpost ?>)</span> | </li>
	<li><a <?php if($show == "organic"): ?>class="current"<?php endif; ?>href="<?php echo $this->_url->linkTo("wpjb/job", "index/show/organic/".$qstring) ?>"><?php _e("Organic", WPJB_DOMAIN) ?></a><span class="count">(<?php echo (int)$stat->organic ?>)</span> | </li>
	<li><a <?php if($show == "import"): ?>class="current"<?php endif; ?>href="<?php echo $this->_url->linkTo("wpjb/job", "index/show/import/".$qstring) ?>"><?php _e("Import", WPJB_DOMAIN) ?></a><span class="count">(<?php echo (int)$stat->import ?>)</span> | </li>
...
<div class="alignleft actions">
   ...
   <!-- <select class="postform" id="sortview" name="sortview">
        <option value="0">View by default</option>
		<option value="1">Sort by view HTL</option>
		<option value="2">Sort by view LTH</option>
    </select> -->
...
<table cellspacing="0" class="widefat post fixed">
    ...
    <<?php echo $tx; ?>>
        <tr>
			...
			<th style="" class="" scope="col"><?php _e("Company", WPJB_DOMAIN) ?></th>
            ...
            <th style="" class="" scope="col"><?php _e("Viewed", WPJB_DOMAIN) ?></th>
        </tr>
    </<?php echo $tx; ?>>
    <?php endforeach; ?>
	...
	<tr valign="top" class="<?php if($i%2==0): ?>alternate <?php endif; ?> author-self status-publish iedit">
            ...
            <!--<td class="author column-author"><?php _e($item->getCategory()->title) ?></td>
            <td class="author column-author"><?php _e($item->getType()->title) ?></td>-->
			<td class="author column-author"><?php if($item->employer_id!=NULL){?> <a href="<?php echo site_url();?>/wp-admin/admin.php?page=wpjb/job&action=index/employer/<?php echo $item->employer_id;?>"><?php echo esc_html($item->company_name);?></a><?php }else echo esc_html($item->company_name);?></td>
