...
function wpjb_jobs_list($atts) {
    ...
    $css = trim(Wpjb_Project::getInstance()->media(), "/") . "/style.css";
    ...
    $view->jobPage = $result->page;
    $view->jobCount = ceil($result->total/$result->perPage);
    $view->jobList = $result->job;

    $router = Wpjb_Project::getInstance()->router();
    $view->cDir = $router->linkTo("search", null, $params);
    
    $qs = $params;
    unset($qs["page"]);
    
    $view->qString = http_build_query($qs);
...
function wpjb_is_new_img($file = "rating_star.jpg")
{
    echo wpjb_img($file);
}

function wpjb_new_img_featured($file = "new_label.png")
{
    echo wpjb_img($file);
}
...
function wpjb_job_company(Wpjb_Model_Job $job = null)
{
    ...
    if(strlen($job->company_website) > 0) {
        ...
        if (!preg_match('#^(?:https?|ftp)://#', $url, $m))
        	$url = 'http://' . $url;
        echo '<a rel ="nofollow" href="'.$url.'" class="wpjb-job-company" target=_blank>'.$company.'</a>';
    } else {
        echo $company;
    }
}
...
function wpjb_contact_description($job)
{
    $job = $job->contact_description;
    $job = strip_tags($job, '<p><a><b><strong><em><i><ul><li><h3><h4><br>');
    $job = nl2br($job);
    $find = array("</ul><br />", "</li><br />", "</ol><br />");
    $repl = array("</ul>", "</li>", "</ol>");
    $text = str_replace($find, $repl, $job);
    $text = autolink( $text, array("target"=>"_blank","rel"=>"nofollow", "style"=>"text-overflow: ellipsis;
    display: inline-block;
    width: 350px;
    white-space: nowrap;
    overflow: hidden;
    vertical-align: top;"));
    //$text = formatUrlsInText($text);
    //echo str_replace($find, $repl, $job);
    echo $text;
}

function wpjb_required_description($job)
{
    $job = $job->job_required;
    $job = strip_tags($job, '<p><a><b><strong><em><i><ul><li><h3><h4><br>');
    $job = nl2br($job);
    $find = array("</ul><br />", "</li><br />", "</ol><br />");
    $repl = array("</ul>", "</li>", "</ol>");
...
function wpjb_interest_description($job)
{
    $job = $job->job_interest;
    $job = strip_tags($job, '<p><a><b><strong><em><i><ul><li><h3><h4><br>');
    $job = nl2br($job);
    $find = array("</ul><br />", "</li><br />", "</ol><br />");
    $repl = array("</ul>", "</li>", "</ol>");
...
function wpjb_job_salary($job)
{
	$cur = $job->sym_currency;
	$job = $job->job_salary;
	if ($cur==2):
		echo $job;
	else :	   
    	echo  number_format($job,0,",",".");
    endif;
}
function wpjb_job_currency($job)
{
    $job = $job->sym_currency;
	if ($job==0):
		$job="USD";
	elseif ($job==1):
		$job="VND";
...
function wpjb_job_contact($job)
{
    $job = $job->job_contact;   
    echo  $job;
}

function wpjb_job_phone($job)
{
    $job = $job->job_phone;   
    echo  $job;
}

function wpjb_job_address($job)
{
    $job = $job->job_address;   
    echo  $job;
}

function job_type($job)
{
	$job = $job->job_type;
	if($job==1)
		echo  __('[:en]Full-Time[:vi]Toàn thời gian[:ja]Full-Time');
	if($job==2)
		echo  __('[:en]Part-time[:vi]Bán thời gian[:ja]Part-time');
	if($job==3)
		echo  __('[:en]Contract[:vi]Hợp đồng[:ja]Contract');
	if($job==4)
		echo  __('[:en]Intern[:vi]Thực tập[:ja]Intern');
	if($job==5)
		echo  __('[:en]Seasonal/Temp[:vi]Thời vụ[:ja]Seasonal/Temp');
	if($job==6)
		echo  __('[:en]Other[:vi]Khác[:ja]Other');
}
...
function wpjb_field_value_return($field)
{
    if($field->getField()->type == 6) {
        $list = '<p><a><b><strong><em><i><ul><li><h3><h4><br>';
        echo nl2br(strip_tags($field->value, $list));
...
function wpjb_render_step($num, $mark = "<strong>{text}</strong>")
{
    $steps = array(
        array(esc_html(Wpjb_Project::getInstance()->conf("seo_step_1")), __("Post a Job", WPJB_DOMAIN)),
...
function wpjb_company_address(Wpjb_Model_Employer $company)
{
    $list = '<p><a><b><strong><em><i><ul><li><h3><h4><br>';
    echo nl2br(strip_tags($company->company_address, $list));
}
function wpjb_company_qual(Wpjb_Model_Employer $company)
{
    $list = '<p><a><b><strong><em><i><ul><li><h3><h4><br>';
    echo nl2br(strip_tags($company->company_qual, $list));
}
function wpjb_company_field(Wpjb_Model_Employer $company)
{
    $list = '<p><a><b><strong><em><i><ul><li><h3><h4><br>';
    echo nl2br(strip_tags($company->company_field, $list));
}
...
function wpjb_company_whyus(Wpjb_Model_Employer $company)
{
    $list = '<p><a><b><strong><em><i><ul><li><h3><h4><br>';
    echo nl2br(strip_tags($company->why_us, $list));
}
// resumes functions
...
function wpjb_resume_id()
{
    // @deprecated
    echo esc_html(wpjb_view("resume")->id);
}
...
{
    if(strtotime($object->updated_at) >= strtotime("1970-01-01 00:00:00")) {
        $exchange = array(
...
function wpjb_form_helper_tinymce(Daq_Form_Element $field, $options = array())
{
    add_filter('the_editor_content', 'wp_richedit_pre');
    wp_editor($field->getValue(), $field->getName(), array("quicktags"=>false, "media_buttons"=>false, "teeny"=>false));
    
}

function wpjb_form_helper_logo_upload(Daq_Form_Element $field, array $options = array())
{
    echo '<input type="file" name="'.$field->getName().'" id="'.$field->getName().'" />';
