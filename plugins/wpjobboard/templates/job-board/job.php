...
<script src="<?php echo plugins_url('js/imgLiquid.js', __FILE__ );?>" type="text/javascript"></script><?php 
...
<script type="text/javascript">
	...
	<table class="wpjb-info">		
            ...
            <tr>
                ...
                <td>
                <?php if(wpjb_conf("show_maps") && $job->getGeo() && $job->getGeo()->lnglat !='14.058324,108.277199' && $job->getGeo()->lnglat !='10.823099,106.629662' && $job->getGeo()->lnglat !='10.746903,106.676292') : ?><a class="modalLink" href="#modal1"><img src="<?php echo wpjb_img("location.png") ?>" alt="" class="wpjb-inline-img" /><?php wpjb_job_address($job)?></a>
                    
				<div id="modal1" class="modal">
					<p class="closeBtn">Close (x)</p>
					<iframe width="498" height="298" style="border: 1px solid #738FE6;" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://maps.google.com/maps?f=q&amp;source=s_q&amp;hl=en&amp;geocode=&amp;q=<?php wpjb_job_address($job)?>&amp;aq=0&amp;oq=orlan&amp;sll=<?php echo $job->getGeo()->lnglat ?>&amp;sspn=0.035898,0.055189&amp;ie=UTF8&amp;hq=&amp;hnear=<?php wpjb_job_address($job)?>&amp;t=m&amp;z=17&amp;ll=<?php echo $job->getGeo()->lnglat ?>&amp;iwloc=near&amp;output=embed"></iframe>
					<!--<div id='googleMap' style='width:499px;height:299px;'></div>-->
				</div>
			<?php else: ?><?php wpjb_job_address($job)?>
			<?php endif; ?>
                </td>
				</tr>
            <?php endif; ?>
...
<?php
	...
	function facebook_meta(){	
		$title = wpjb_view("job")->job_title;
		$desc = wpjb_view("job")->job_description;
		if (wpjb_view("job")->getEmployer(true)->getFileUrl())
			$img = wpjb_view("job")->getEmployer(true)->getFileUrl();
		else if (wpjb_view("job")->getImageUrl())
			$img = wpjb_view("job")->getImageUrl();
		else $img = get_template_directory_uri()."/images/sagojo_fb.png";
