...
jQuery(function() {
	...
	if(jQuery("#job_location")) {
        ...
        var altState = jQuery('<input type="text" />')
            ...
            .attr("name", "wpjb_location_city");
			
		var opt = jQuery('<select><option value="Tokyo">Tokyo</option><option value="Kyoto">Kyoto</option><select />')
			.hide()
            .attr("id", "job_location_jp")
            .attr("value", Wpjb.Location)
            .attr("name", "wpjb_location_city")
	
        jQuery("#job_location").after(altState,opt);

        jQuery("#job_country").bind("change", function() {
            var stateSelect = jQuery("#job_location");
			var opt = jQuery("#job_location_jp");
            var stateInput = jQuery("#job_location_text");
            if(jQuery(this).val() != 704&&jQuery(this).val() != 392) {
                stateInput.show();
                stateSelect.hide();
				opt.hide();
            } else {
	...
	if(jQuery("#company_location")) {
        ...
        jQuery("#company_location").after(altState,opt);
