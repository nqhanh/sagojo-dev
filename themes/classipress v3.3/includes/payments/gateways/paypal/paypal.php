...
if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
	require dirname( __FILE__ ) . '/paypal-test.php';
}
...
class APP_PayPal extends APP_Boomerang{
	...
	public function register(){
		...
		if( !empty( $options['ipn_enabled'] ) )
			$this->listener = new APP_PayPal_IPN_Listener( array( $this->bridge, 'handle_response' ) );
	...
	public function create_form( $order, $options ) {
		...
		list( $form, $fields ) = APP_PayPal_Form::create_form( $order, $options, $return_url, $cancel_url );
