...
class APP_Order {
	 ...
	 * @param string $type    A string representing the type of item being removed
	 * @param int $price  	  The price of the item being removed (optional)
	 * @param int $post_id    The post that this item affects (optional)
	 */
	public function remove_item( $type = '', $price = '', $post_id = '' ) {
	...
	public function activate() {
		...
		if( $this->is_recurring() ){
			...
			$order = APP_Order_Factory::duplicate( $this );
			$date = date( 'Y-m-d H:i:s', strtotime( '+' . intval( $this->get_recurring_period() ) . ' days' ) );
			$order->schedule_payment( $date );
