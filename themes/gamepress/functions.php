...
function gamepress_enqueue_scripts()
{
	...
	if (is_home() && of_get_option('gamepress_slider_radio')){
...
add_action( 'load-themes.php', 'gamepress_flush' );

/* Flush your rewrite rules */
function gamepress_flush() {
...
/*-----------------------------------------------------------------------------------*/
function gamepress_post_pagination($content) {
    
    if ( is_single() ) {
        $content .= wp_link_pages( array( 'before' => '<div class="page-link">' . __( 'Pages:', 'gamepress' ), 'after' => '</div>', 'echo' => 0 ) );
    }
	return $content;
}
add_filter('the_content','gamepress_post_pagination', 10);
...
function gamepress_add_button() {  
   ...
   {  
     ...
     add_filter('mce_buttons', 'gamepress_register_button');  
   }  
}  
