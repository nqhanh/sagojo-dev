jQuery(document).ready(function(){
	...
	/*
	 * AT_Options_upload function
	 * Adds media upload functionality to the page
	 */

	var header_clicked = false;

	jQuery("img[src='']").attr("src", athemes_upload.url);

	jQuery('.athemes-opts-upload').click(function() {
		header_clicked = true;
	...
	jQuery('.athemes-opts-upload-remove').click(function(){
		$relid = jQuery(this).attr('rel-id');
		jQuery('#'+$relid).val('');
		jQuery(this).prev().fadeIn('slow');
		jQuery(this).prev().prev().fadeOut('slow', function(){jQuery(this).attr("src", athemes_upload.url);});
