...
function maketing_mails($meta_key, $meta_value)
{
	/*if ( 'publish' !== $new_status or 'publish' === $old_status
	or 'freelance_post' !== get_post_type( $post ) )
		return;*/

	$employer = array ();//lay danh sach nha tuyen dung
	if ($meta_value == 1){
		foreach( get_users_by_meta_data ($meta_key, $meta_value) as $user ) {
			array_push($employer, $user->ID);
		} // end foreach
	}
	if ($meta_value == 1){
		$subscribers = get_users( array ( 'role' => 'editor', 'exclude' =>  $employer ) );//gui email cho subcribers, loai tru nha tuyen dung
		$emails      = array ();
		$name = array ();
	}
	if ($meta_value == 0){
		$subscribers = get_users( array ( 'role' => 'Subscriber', 'meta_key'  => 'is_employer', 'meta_value' => '1' ) );//gui email cho nha tuyen dung
		$emails      = array ();
		$name = array ();
	}
	...
	foreach ( $subscribers as $subscriber ){
		...
		array_push($name, $subscriber->display_name);
		//$emails[] = $subscriber->user_email;
		//$name[] = $subscriber->display_name;
	}
	if ($meta_value == 1){
		$message_template = file_get_contents(ABSPATH.'/wp-content/themes/responsive/magazine-email.html');
	}
	if ($meta_value == 0){
		$message_template = file_get_contents(ABSPATH.'/wp-content/themes/responsive/employer-email.html');
	}
	$body = str_ireplace('[site_url]',site_url(), $message_template);
	
	
	$post_results="SELECT wp_posts.* FROM wp_posts 
					WHERE 1=1 
					AND wp_posts.post_type = 'post' 
					AND (wp_posts.post_status = 'publish')
					ORDER BY wp_posts.post_date DESC
					LIMIT 0 , 30";
	$post_result=mysql_query($post_results);
	
	if(mysql_num_rows($post_result)>0)
	{
		$i=1;
		while($posts=@mysql_fetch_assoc($post_result))
		{
		if ($i==1){
			$link1 = $posts['guid'];
			$title1 = $posts['post_title'];
			$except1 = st_substr($posts['post_content'],300);
			$largeimage = get_the_post_thumbnail( $posts['ID'], 'large-image' );
			//$largeimage = get_the_post_thumbnail($posts['ID'], array(660,210) );
			
		}
		elseif ($i==2){
			$link2 = $posts['guid'];
			$title2 = $posts['post_title'];
			$except2 = st_substr($posts['post_content'],100);
			$image2 = get_the_post_thumbnail( $posts['ID'],'small-image');	
		}
		elseif ($i==3){
			$link3 = $posts['guid'];
			$title3 = $posts['post_title'];
			$except3 = st_substr($posts['post_content'],100);
			$image3 = get_the_post_thumbnail( $posts['ID'],'small-image');
		}
		elseif ($i==4){
			$link4 = $posts['guid'];
			$title4 = $posts['post_title'];
			$except4 = st_substr($posts['post_content'],100);
			$image4 = get_the_post_thumbnail( $posts['ID'],'small-image');
		}		
		$i++;
	}
	}
	
	$body = str_ireplace('[link1]',$link1, $body);
	$body = str_ireplace('[title1]',$title1, $body);
	$body = str_ireplace('[except1]',$except1, $body);
	$body = str_ireplace('[largeimage]',$largeimage, $body);
	
	$body = str_ireplace('[link2]',$link2, $body);
	$body = str_ireplace('[title2]',$title2, $body);
	$body = str_ireplace('[except2]',$except2, $body);
	$body = str_ireplace('[image2]',$image2, $body);
	
	$body = str_ireplace('[link3]',$link3, $body);
	$body = str_ireplace('[title3]',$title3, $body);
	$body = str_ireplace('[except3]',$except3, $body);
	$body = str_ireplace('[image3]',$image3, $body);
	
	$body = str_ireplace('[link4]',$link4, $body);
	$body = str_ireplace('[title4]',$title4, $body);
	$body = str_ireplace('[except4]',$except4, $body);
	$body = str_ireplace('[image4]',$image4, $body);
	
	
	$results="SELECT * FROM wpjb_job ORDER BY wpjb_job.id DESC limit 8";
	...
	if(mysql_num_rows($result)>0)
	{
		...
		{
			if ($j==1){
				$job_slug1 = $row['job_slug'];
				$job_title1 = $row['job_title'];
				$company_name1 = $row['company_name'];
			}
			elseif ($j==2){
				$job_slug2 = $row['job_slug'];
				$job_title2 = $row['job_title'];
				$company_name2 = $row['company_name'];
			}
			elseif ($j==3){
				$job_slug3 = $row['job_slug'];
				$job_title3 = $row['job_title'];
				$company_name3 = $row['company_name'];
			}
			elseif ($j==4){
				$job_slug4 = $row['job_slug'];
				$job_title4 = $row['job_title'];
				$company_name4 = $row['company_name'];
			}
			elseif ($j==5){
				$job_slug5 = $row['job_slug'];
				$job_title5 = $row['job_title'];
				$company_name5 = $row['company_name'];
			}
			elseif ($j==6){
				$job_slug6 = $row['job_slug'];
				$job_title6 = $row['job_title'];
				$company_name6 = $row['company_name'];
			}
			elseif ($j==7){
				$job_slug7 = $row['job_slug'];
				$job_title7 = $row['job_title'];
				$company_name7 = $row['company_name'];
			}
			elseif ($j==8){
				$job_slug8 = $row['job_slug'];
				$job_title8 = $row['job_title'];
				$company_name8 = $row['company_name'];
			}
			$j++;
		}
	}

	$body = str_ireplace('[job_slug1]',$job_slug1, $body);
	$body = str_ireplace('[job_title1]',$job_title1, $body);
	$body = str_ireplace('[company_name1]',$company_name1, $body);
	
	$body = str_ireplace('[job_slug2]',$job_slug2, $body);
	$body = str_ireplace('[job_title2]',$job_title2, $body);
	$body = str_ireplace('[company_name2]',$company_name2, $body);
	
	$body = str_ireplace('[job_slug3]',$job_slug3, $body);
	$body = str_ireplace('[job_title3]',$job_title3, $body);
	$body = str_ireplace('[company_name3]',$company_name3, $body);
	
	$body = str_ireplace('[job_slug4]',$job_slug4, $body);
	$body = str_ireplace('[job_title4]',$job_title4, $body);
	$body = str_ireplace('[company_name4]',$company_name4, $body);
	
	$body = str_ireplace('[job_slug5]',$job_slug5, $body);
	$body = str_ireplace('[job_title5]',$job_title5, $body);
	$body = str_ireplace('[company_name5]',$company_name5, $body);
	
	$body = str_ireplace('[job_slug6]',$job_slug6, $body);
	$body = str_ireplace('[job_title6]',$job_title6, $body);
	$body = str_ireplace('[company_name6]',$company_name6, $body);
	
	$body = str_ireplace('[job_slug7]',$job_slug7, $body);
	$body = str_ireplace('[job_title7]',$job_title7, $body);
	$body = str_ireplace('[company_name7]',$company_name7, $body);
	
	$body = str_ireplace('[job_slug8]',$job_slug8, $body);
	$body = str_ireplace('[job_title8]',$job_title8, $body);
	$body = str_ireplace('[company_name8]',$company_name8, $body);
	...
	get_option( 'user_email' ),
	'Bản tin sagojo',
	$body,
	// extra headers
	array (
	'Bcc:' . implode( ",", $emails ),
	'From: [sagojo] <postmaster@aline.jp>',
	)
	);
