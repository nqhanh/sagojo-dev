			...
			<form method="post" id="personal-form-id-authorinfo" class="personal-form-class-authorinfo">
					...
					<div class="personal-input">
						<?php
							...
							if (file_exists($country_file) && is_readable($country_file))
							{
								...
								<option value="<?php echo stripslashes(get_user_meta($auth->id ,'country' ,TRUE)); ?>"><?php _e(stripslashes(get_user_meta($auth->id ,'country' ,TRUE))); ?></option>
								<?php
								foreach ($countrys as $country) {
									if ($country['name']==$_SESSION['country'])
										{
										?>
										<option value="<?php echo $country['name']; ?>" selected><?php _e($country['name']) ?></option>
										<?php
										}
									else{
										?>
										<option value="<?php echo $country['name']; ?>"><?php _e($country['name']) ?></option>
										<?php
										}
									}
									?>
							...
							else
							{
								...
								die("Sorry, the $country_file file doesnt seem to exist or is not readable!");
							}
							?>
				</div>
				</div>				
				...
				<div id="personal-city" class="personal-profile">
					...
					<?php
							...
							if ($cid == ''){
								$_SESSION['country'] = stripslashes(get_user_meta($auth->id ,'country' ,TRUE));
								$cid = $_SESSION['country'];
								}
							if ($cid == 'Vietnam')
