...
class Rating {
  ...
  public $wp_rtgitems = 'wp_rtgitems';        // Table /or file_name to store items that are ranted
  public $wp_rtgusers = 'wp_rtgusers';             // Table /or filename that stores the users who rated in current day
  ...
  public function __construct() {
    ...
    if($this->svrating != 'mysql') {
      $this->wp_rtgitems = '../ratingtxt/'.$this->wp_rtgitems.'.txt';
      $this->wp_rtgusers = '../ratingtxt/'.$this->wp_rtgusers.'.txt';
    }
  }
  ...
  public function getRating($items, $rate = '') {
    ...
    if(!empty($rate)) {
      ...
      if($this->rater === '')
	  return "alert('You can rate this user when this Job-offer has closed down.')";	  
      else {
    ...
    $wp_rtgitems = ($this->svrating == 'mysql') ? $this->getRtgDb($items, $rtgstdy, $setrated) : $this->getRtgTxt($items, $rtgstdy, $setrated);

    return json_encode($wp_rtgitems);
  }

  // insert /update rating item in #wp_rtgitems, delete rows in $wp_rtgusers which are not from today, insert $rater in $wp_rtgusers
  protected function setRtgDb($items, $rate, $rtgstdy) {
    $this->sqlExecute("INSERT INTO `$this->wp_rtgitems` (`item`, `totalrate`) VALUES ('".$items[0]."', $rate) ON DUPLICATE KEY UPDATE `totalrate`=`totalrate`+$rate, `nrrates`=`nrrates`+1");

    $this->sqlExecute("DELETE FROM `$this->wp_rtgusers` WHERE `day`!=$this->tdy");

    $this->sqlExecute("INSERT INTO `$this->wp_rtgusers` (`day`, `rater`, `item`) VALUES ($this->tdy, '$this->rater', '".$items[0]."')");
  ...
  protected function getRtgDb($items, $rtgstdy, $setrated) {
    ...
    $resql = $this->sqlExecute("SELECT * FROM `$this->wp_rtgitems` WHERE `item` IN(".implode(',', array_map('addSlhs', $items)).")");
  ...
  // add /update rating item in TXT file, keep rows from today in $wp_rtgusers, and add new row with $rater
  protected function setRtgTxt($items, $rate, $all_rtgstdy) {
    // get the rows from file with items, if exists
    if(file_exists($this->wp_rtgitems)) {
      $rows = file($this->wp_rtgitems, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
      $nrrows = count($rows);
    ...
    file_put_contents($this->wp_rtgitems, implode(PHP_EOL, $rows));      // save the items in file
    ...
    file_put_contents($this->wp_rtgusers, implode(PHP_EOL, $all_rtgstdy['all']));

    // add curent rated item to the others today, and save them as string ',' in cookie (till the end of day)
    $all_rtgstdy[$this->tdy][] = $items[0];
    setcookie("ratings", implode(',', array_unique($all_rtgstdy[$this->tdy])), strtotime('tomorrow'));
  }
  ...
  protected function rtgstdyDb() {
    $rtgstdy = array();
    $resql = $this->sqlExecute("SELECT `item` FROM `$this->wp_rtgusers` WHERE `day`=$this->tdy AND `rater`='$this->rater'");
    if($this->affected_rows > 0) {
      for($i=0; $i<$this->affected_rows; $i++) {
  ...
  protected function rtgstdyTxt() {
    ...
    if(file_exists($this->wp_rtgusers)) {
      $rows = file($this->wp_rtgusers, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
      $nrrows = count($rows);
