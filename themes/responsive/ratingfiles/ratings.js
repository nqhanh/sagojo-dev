...
function rateStars(spn) {
  ...
  if(itemrated_rtg == '' && spn.parentNode.parentNode.parentNode) itemrated_rtg = spn.parentNode.parentNode.parentNode.innerHTML;       // store the rating of rated item
    ...
    if (navigator.userAgent.indexOf("Firefox")!=-1) star_sp.setAttribute('style', 'width:'+((i_sp+1)*22)+'px; background:url("http://192.168.0.105/wordpress_op2/wp-content/themes/responsive/ratingfiles/star2.png")');
    else {
      star_sp.style.width = ((i_sp+1)*22)+'px';
      star_sp.style.background = 'url("http://192.168.0.105/wordpress_op2/wp-content/themes/responsive/ratingfiles/star2.png")';
    }
  }
}
...
function addRtgData(elm, totalrate, nrrates, renot) {
  ...
  var htmlrtg = '<div class="totalrate"><i>Rating:</i>'+avgrating+'/5</div><div class="stars"><div class="star_n" style="width:'+star_n+'px;">&nbsp;</div>'+d_rtg+'</div><em>'+nrrates+' votes</em>'
...
function rtgAjax(elm, ratev) {
  ...
  cerere_http.open("POST", 'http://192.168.0.105/wordpress_op2/wp-content/themes/responsive/ratingfiles/ratings.php', true);			// crate the request
